...
def areCloseEnough(p1, p2):
    x1, y1, z1 = p1
    x2, y2, z2 = p2
    return abs(x1 - x2) <= 0.1 and abs(y1 - y2) <= 0.1 and abs(z1 - z2) <= 0.1
...
def compareFiles(f1, f2):
    ...
    for resid in original:
        ...
        if len_orig != len_new:
            print(
                "Error in res #{0} {1}: {2} {3}: {4}".format(
                    resid, f1, len_orig, f2, len_new
                )
            )
            problems = True
            for i in res:
                if i not in resnew:
                    print("{0} not in {1}".format(i, f2))
            for i in resnew:
                if i not in res:
                    print("{0} not in {1}".format(i, f1))

        for atom in res:
            if atom not in resnew:
                print("{0} not in {1}{2} {3}".format(atom, res[atom][0], resid, f2))
                problems = True
                continue
            (resname1, x1, y1, z1) = res[atom]
            (resname2, x2, y2, z2) = resnew[atom]
            if resname1 != resname2:
                print("Warning: resname {0} != {1}".format(resname1, resname2))
                problems = True
        ...
        for atom in resnew:
            if atom not in res:
                print("{0} not in {1} {2}{3}".format(atom, f2, resnew[atom][0], resid))
                problems = True
                continue
    return problems
