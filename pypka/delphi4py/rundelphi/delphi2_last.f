c###############################################################################
	subroutine delphi(acent_xdim, energy_xdim, site_xdim,
     ...
     & py_in_pdb, py_in_crg, py_in_siz, py_in_frc,
     & py_in_pdb_len, py_in_crg_len, 	
     & py_in_siz_len, natom, nmedia, nobject,
     & py_i_atpos, py_i_rad3, py_i_chrgv4, py_i_atinf, py_i_medeps,
     & py_i_iatmmed, py_i_dataobject, py_i_phimap4,
     & py_out_phi, py_i_sitpot, esolvation)	
c
c     & py_rad3, rad3_xdim, py_chrgv4, chrgv4_xdim, py_atinf, atinf_xdim,
c     & py_medeps, medeps_xdim, py_iatmmed, iatmmed_xdim, py_polariz, 
c       & polariz_xdim
...
c       py_variables are not used directly but rather reassigned
	integer acent_xdim, energy_xdim, site_xdim
	integer py_igrid
	real*8  py_scale
	real*8  py_repsin
	real*8  py_repsout
	...
	character*(*) py_energy(energy_xdim), py_site(site_xdim)	
        character*(*) py_in_pdb, py_in_crg, py_in_siz, py_in_frc
	integer py_in_pdb_len, py_in_crg_len, py_in_siz_len
	integer natom
	integer nmedia
	integer nobject
...
Cf2py   intent(in) py_in_pdb_len
Cf2py   intent(in) py_in_crg_len
Cf2py   intent(in) py_in_siz_len
...
c       jbc !!!!!!!!!!!!!!!!!! New GPU-enabled solver !!!!!!!!!!!!!!!  jbc
	...
	useNanoShaper = 0      
	solverModuleFile = 'libPbsolver4Delphi'//CHAR(0)
c jbc !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
c------------------------------------------------------------------
	icount1a=0
	icount1b=0
...
c
	ias=1
	pdbnam=py_in_pdb
	pdblen=py_in_pdb_len

	crgnam=py_in_crg
	crglen=py_in_crg_len

	siznam=py_in_siz
	sizlen=py_in_siz_len

	PRINT *, 'frc name ->', py_in_frc
...
! write custom settings file for NanoShaper 
	   open (unit = 010, file = "custom.prm")
	   if(isurftype.eq.0)  write(010,*), "Surface = ses"
	   if(isurftype.eq.1)  write(010,*), "Surface = skin"
	   if(isurftype.eq.2)  write(010,*), "Surface = blobby"
	   if(isurftype.eq.3)  write(010,*), "Surface = mesh"
	   if(isurftype.eq.4)  write(010,*), "Surface = msms"
	   close(010)
	endif
...
c       e++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &  ibctype,isolvarch,rionst,igrid,scale,nlit,res2,iper,qplus,cqplus,qmin,cqmin)
     ...
     &	icount1a,icount1b,spec,i_phimap,i_phimap1,i_phimap2,i_phimap3,
     &	i_ibndx,i_ibndy,i_ibndz,i_bndx1,i_bndx2,i_bndx3,i_bndx4,i_gval,
     &  ibctype,isolvarch,rionst,igrid,scale,nlit,res2,i_idebmap,imanual,nnit,ival,ival2,
     &  conc,i_qmap1,i_qmap2,qfact,epsout,deblen,relpar)
