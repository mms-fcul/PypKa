	subroutine extrmobjects(nobject,scale,natom,numbmol,verbose)
...
c here we have a molecule, calculating also max radius
              numbmol=numbmol+1
              if(verbose)write(6,*)
     &          "object number :   ",count,",  is a molecule"    
              if (natom.eq.0) then
...
c                 alpha=alpha*3.1415/180.
     ...
     &   geometric parameter too small to be 'resolved' by the program"
                 do i=1,3
                  tmp1=tmpvect(i)*tmpvect(i)
                  tmp=tan(alpha*.5)*sqrt(modul2-tmp1)
                  limobject(count,i,1)=min(xb(i),xa(i)-tmp)
                  limobject(count,i,2)=max(xb(i),xa(i)+tmp)
                 end do
              end if
