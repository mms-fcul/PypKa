...
c re-entrant mid-points later (15 Aug 93)
	...
	logical itobig,itest2,ipore,ionlymol,isitmd,debug,verbose
      if(verbose)write(6,*)
...
CC	if(itest2.eqv..true.) write(6,*)iv,xn,rad3(iv),ismin,radmax2
	  end do
c
...
c faster method
c IT HAS PROBLEMS!!!! Walter (be careful before using also in multidilectric case!!!.and..not.isitmd
	    rad2a = rad2 - 0.25
	    ixn1=nint(xn(1))
	    iyn1=nint(xn(2))
	    izn1=nint(xn(3))
...
c           do iz=1,igrid
           iz=(igrid+1)/2
              write (52,*)ix,iy,iz,iepsmp(ix,iy,iz,3)
c           end do
         end do
       end do
      close (52)
