	subroutine setbc(qplus,qmin,cqplus,cqmin,nqass,
     &  natom,ibnum,py_scale1)
c
...
c--------------------------------------------------------------
	...
	character*16 botlbl
        ...
        real*4 scalesingle, oldmidsingle(3)
c--------------------------------------------------------------
        integer ibnum
...
c
c	open(18,file=phiinam(:phiilen),status='old',
c     1err=900,form='unformatted')
	write(6,*)' '
	write(6,*)'focussing boundary condition '
c	write(6,*)'read from file'
c	write(6,*)phiinam(:phiilen)
	write(6,*)' '
c
c read in old potential map
c
c	read(18)toblbl
c	read(18)label,title
...
c	   read(18) phimap4
	   ...
	   do iz=1,igrid
	      do iy=1,igrid
		 do ix=1,igrid
		    phimap(ix,iy,iz)=phimap4(i)
		    if ((i .gt. 180) .and. (i .lt. 190)) then
...
c	   i_phimap4=memalloc(i_phimap4, 0,0)
c	   read(18) botlbl
c	   read (18) scalesingle,oldmidsingle, igrid
c       scale1=scalesingle
	   scale1=py_scale1
	   do ii=1,3
c	      oldmid1(ii)=oldmidsingle(ii)
	      oldmid1(ii)=acent(ii)
	   enddo
c Emil_May_2011
	   igrid1=igrid
cc // phimap in DELPHI phi file always real*4, so are scale and oldmid
CC that''s why the transfer from phimap4 to phimap
c  e++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c	else
c	   read(18)phimap
c	   read(18)botlbl
c       read(18)scale1,oldmid1,igrid1
...
c Emil_May_2011
	   igrid1=igrid
	   PRINT *, igrid1, oldmid1, scale1, py_scale1
	endif

c	close(18)

c       check to see if this is a continuation
...
c	write(6,200)title
c	write(6,*)'original scale (grids/A)      : ',scale1
c	write(6,*)'object centre at (A) : ',oldmid1
c	write(6,*)' '
 200	format(A60)
c
...
c
	if(ibctyp.eq.5) then
	fact=0.
	subt=vdropz*(fact-1.)/2.
c       fixed potential over z direction
        do 9060 iz=2,igrid-1
          do 9062 ix=1,igrid
            do 9061 iy=1,igrid
c               phimap(ix,iy,iz) = vdropz*(iz-1)/(igrid-1.)
      phimap(ix,iy,iz)=vdropz*fact/(1.+exp((-2.*iz+igrid+1.)/35.5))-subt
c              phimap(ix,iy,iz) = vdropz/(1.+exp(-2.*iz+igrid+1.))
9061        continue
9062	  continue
9060	continue
...
c---------------------------------------------------------------
	midg = (igrid+1)/2
	if(verbose) then
	   write(6,*)' some initial phi values: ', midg, igrid
