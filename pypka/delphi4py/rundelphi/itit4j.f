...
c
	...
	dimension ibndx(nbgp),ibndy(nbgp),ibndz(nbgp),grdn(5)
	character*24 day
	character*1 symb
...
c-------------------------------------------------------
        start = cputime(0.0)
        call datime(day)
        if(verbose)write(6,*)'now iterating at: ',day(12:19)
c-------------------------------------------------------
...
c
CC bndx 1,2,3,4 seem to form a square for any given j,k, in phimap(i,j,k) 
c
1000    continue
c
...
c         fin-star=igrid*(igrid-3)*(igrid+1)/2
          ...
          ix=1+igrid*lat2-igrid
	    ishift=igrid*lat1
          if(rionst.gt.0.0) then
            do 2000 n=1,(igrid-3)*lat2+1
              ix=ix+igrid
c scandisco tutti i punti, anche i bordi (in questa fase del ciclo ho solo spigoli xy)
c e li scarto a posteriori
              itest=mod(n,igrid)
              if(itest.eq.0..or.itest.eq.lat2) then
			  if (iper(2)) then
                  if(itest.eq.0.)then
	              temp2 = phimap2(ix+lat1-ishift)+phimap2(ix-lat2)
	            else
	              temp2 = phimap2(ix+lat1)+phimap2(ix-lat2+ishift)
	            end if
			  else 
			    goto 2000
	          end if
...
c e+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
CC if (iper(1) .and..false)) // end of the block.
...
c e li scarto a posteriori
			  ...
			  if (iper(2)) then
                  if(itest.eq.0.)then
				  temp2=phimap1(ix-lat1+ishift)+phimap1(ix+lat2)
	            else
	              temp2=phimap1(ix-lat1)+phimap1(ix+lat2-ishift)
	            end if
			  else 
			    goto 2200
	          end if
...
c        write(*,*)"pot112:",phimap(1,1,2)
c        write(*,*)"pot113:",phimap(1,1,3)
c        write(*,*)"pot123:",phimap(1,2,3)
        end if
c ++da cacciare!!++++++++++++++++++++++++++++++++
       if (.false.) then
	write(6,*)"lo faccio"
...
c	write(6,*)"Green(-10,-10,-10)",Green(-10,-10,-10)
c       open(52,file='Green10.bin',form='unformatted')
c	   read(52)Green1
c       close(52)
c	write(6,*)"Green1(-10,-10,-10)",Green1(-10,-10,-10)
c       open(52,file='Green1.dat',form='formatted')
c         do ix=-10,10
c	   do iy=-10,10
c	   do iz=-10,10
c            write(52,'(i3,i3,i3,2f10.7)')ix,iy,iz,Green1(ix,iy,iz),
c     &Green(ix,iy,iz)
c	   end do
c         end do
c         end do
...
c	   do iy=-10,10
c	   do iz=-10,10
c            write(6,*)Green1(ix,iy,iz)-Green(ix,iy,iz)
c	   end do
c         end do
c         end do
       end if
...
c
	if(diff.and.(once)) then
	  ...
	  once=.false.
c	  om3= 2.0/(1 +(3.14159/float(igrid)))
	  om3= 2.0/(1 +(3.14159265359/float(igrid))) !LinLi test
	  sixth=sixth*om3/om2
...
8022	  continue 
	    ...
	    icount2a=0
	    icount2b=0
c	    nlit=int(7.8*float(igrid)/3.14159)
	    nlit=int(7.8*float(igrid)/3.14159265359)
	    i=1
	  goto 1689
	end if 
c
