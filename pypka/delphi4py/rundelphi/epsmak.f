	subroutine epsmak(ibnum,natom,oldmid,uniformdiel,
...
c b+++++++++
        ...
        i_limgunit=memalloc(i_limgunit,realsiz,6*nobject)
        ibmx=50000000 !Lin Li: ibmx used to be 1000000, sometimes not enough.
c +++++++++here limobject is expressed in grid units++++
	...
	PRINT *, 'radmax2 ->', scale, radmax2, temp, itest2
...
c
	...
	if(verbose)write(6,*) 'start vw surface at ',finish
	
	PRINT *, 'radmax2 ->', scale, radmax2, temp, itest2
	
	call setout(natom,0.0,exrad,scale,igrid,nobject,oldmid,nmedia,
     &ionlymol,isitmd,debug,verbose)
	finish=cputime(start)
	if(verbose)write(6,*) 'fill in re-entrant regions at ',finish 
c       i_ibnd= memalloc(i_ibnd,4*3*ibmx)
        i_limgunit=memalloc(i_limgunit,0,0)
	call vwtms(ibnum,atpos,natom,oldmid,limeps,nmedia
     &  ,nobject,ibmx,numbmol)
c       i_ibnd= memalloc(i_ibnd,0)

	finish=cputime(start)
	if(.not.isrf.and.verbose)
