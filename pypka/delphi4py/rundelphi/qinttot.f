...
c
	...
	integer arglen
c
	!!!!!!!!!!!!!! NanoShaper default value !!!!!!!!!!
	isurftype=-1

	call defprm
	
!       write(*,*) "INSIDE",isurftype
...
c e+++++++++++++++++++++++++++++++++++++++++
c jbc+++++++++++++++++++++++++++++++++++++++
	if(type(1:ieq1).eq."SOLVER") itype=20
c jbc+++++++++++++++++++++++++++++++++++++++
c sd++++++++++++++++++++++++++++++++++++++++
	if(type(1:ieq1).eq."SURFACE") itype=30
c sd++++++++++++++++++++++++++++++++++++++++
...
c
	if(index(direc(1:ieq2),"(PDB").ne.0.or.index(direc(1:ieq2),"(MODPDB").ne.0) then !LinLi
	pbdfrm=0
...
c ++++++++++++++LinLi:add modpdb option:+++++++++++++++
	if(index(direc(1:ieq2),"(MODPDB4").ne.0) then !LinLi
	pbdfrm=40
	if(ij3) call rdflnm(j1,j2,direc2,pdbnam,pdblen)
	if(ifrm) then
...
c
	...
	if(ifrm) then
c b++++++++++++++++++++++++++++++++++++++++++++
          if(frmnam(:frmlen).eq."XYZ") frcifrm=1
c e++++++++++++++++++++++++++++++++++++++++++++
...
c b++++++importing meshed surface file+++++++++++
        if(index(direc(1:ieq2),"(MESHFILE").ne.0) then
        if(ij3) call rdflnm(j1,j2,direc2,meshnam,meshlen)
        end if
c e+++++++++++++++++++++++++++++++++++++++++++++
c
	end if
...
c b+++++++++++++++++++++++++++++++++++++++++++
	  if(frmnam(:frmlen).eq."CCP4")   phifrm=3
        ...
        if(frmnam(:frmlen).eq."ASCII")   phifrm=6
c e+++++++++++++++++++++++++++++++++++++++++++
...
c b++++++++++++++++++++++++++++++++++++++++++++++
      if(frmnam(:frmlen).eq."XYZN")ivertnorm=.true.
c e++++++++++++++++++++++++++++++++++++++++++++++
	end if
	end if
c
...
c
	if(index(direc(1:ieq2),"(EPS").ne.0) then
	  ...
	  if(ifrm) then
c ++++++++++WWW+++++++++++
	    if(frmnam(:frmlen).eq."EXT") epsfrm=1
c      extended (i.e. non compact) iepsmp format 
c ++++++++++WWW+++++++++++
	    if(epsfrm.eq.0) then
...
c b++++++++++++++++++++++++++++++++++++++++++++++++++
c     saving debyemap
	if(index(direc(1:ieq2),"(DEB").ne.0) then
	  idebwrt=iflag
	  if(ij4) call rdflnm(j1,j2,direc2,debnam,debnamlen)

	end if
c
	if(index(direc(1:ieq2),"(MODPDB").ne.0) then
c          print*,"LinLi: ieq2",ieq2,direc(1:ieq2)
             
	  iatout=iflag
	  mpdbfrm=0
            if(index(direc(1:ieq2),"(MODPDB4").ne.0) mpdbfrm=40 !Lin : 4 digit precision
	  if(ij4) call rdflnm(j1,j2,direc2,mpdbnam,mpdblen)

c                print*,"LinLi: mpdbfrm ",mpdbfrm
	  if(ifrm) then
c ++++++++++WWW+++++++++++
	     if(frmnam(:frmlen).eq."PQR") mpdbfrm=1
	     if(frmnam(:frmlen).eq."MOD") mpdbfrm=0
              if(index(direc(1:ieq2),"(MODPDB4").ne.0.and.frmnam(:frmlen).eq."PQR") mpdbfrm=41 !Lin : 4 digit precision with PQR
              if(index(direc(1:ieq2),"(MODPDB4").ne.0.and.frmnam(:frmlen).eq."MOD") mpdbfrm=40 !Lin : 4 digit precision with PQR
             
c ++++++++++WWW+++++++++++
c                print*,"LinLi: mpdbfrm with pqr",mpdbfrm
...
c jbc++++++++++++++++++++++++++++++++++++++++++++++++++++
        if(itype.eq.20) then
	      if(index(direc(1:ieq2),"DELPHI").ne.0) isolvarch=0
	      if(index(direc(1:ieq2),"GPU").ne.0) isolvarch=1
	      if(index(direc(1:ieq2),"CPP").ne.0) isolvarch=2
	     end if
c jbc++++++++++++++++++++++++++++++++++++++++++++++++++

c sd+++++++++++++++++++++++++++++++++++++++++++++++++++++
    ! NanoShaper surface keyword
      if(itype.eq.30) then
!	     write(*,*) direc(1:ieq2)
         if(index(direc(1:ieq2),"(CONNOLLY").ne.0)  then
!           write(*,*) "Connolly!"
		   isurftype=0
         end if 
         if(index(direc(1:ieq2),"(SKIN").ne.0) then
!            write(*,*) "Skin!" 
		   isurftype=1 
         end if 
         if(index(direc(1:ieq2),"(BLOBBY").ne.0) then
!            write(*,*) "Blobby!" 
		   isurftype=2
         end if
         if(index(direc(1:ieq2),"(MESH").ne.0) then
            !write(*,*) "Mesh!" 
		   isurftype=3
         end if
         if(index(direc(1:ieq2),"(MSMS").ne.0) then
!            write(*,*) "Msms!" 
		   isurftype=4
         end if
      end if
...
c
c
c	inquire(file=prmnam(:prmlen),exist=iext)
c	if(.not.iext) then
c	write(6,*) "parameter file ",prmnam(:prmlen)," does not seem to 
c     &  exist"
c        write(6,*)'Therefore stopping'
c        stop
c	goto 1000
c	end if
cc
c	write(6,*) "opening parameter file ",prmnam(:prmlen)
c	open(10,file=prmnam(:prmlen))
cc
cc set current file length to prmlen
cc set current file to prmnam
c	ifn=10
c	ifillen=prmlen
c	filnam(:ifillen)=prmnam(:prmlen)
cc
cc goto here if a new file has just been opened
cc and reset line number to 1 (only used by type 1 files)
cc
c10	itype(ifn)=0
c	lin1(ifn)=1
cc
cc goto here to read a new line from an already opened file
cc
c20	line=" "
cc	write(6,*) "reading line"
c	read(ifn,204,err=500,end=100) line
cc added 9/9/92 to fixed qincludes being not commented out!
c	if(line(1:1).eq."!") line(2:80)=" "
cc
cc do below if the type of the prm file is undetermined
cc normally the first line, unless that was a qinclude
cc statement
cc
cc	write(6,*) "checking for type0"
c	if(itype(ifn).eq.0) then
cc check as to whether the first character of the file is a number
cc	write(6,*) "checking for type"
c	if(index(cnumb,line(1:1)).eq.0) then
c	j1=index(line,"QINCLUDE")
c	j2=j1+index(line,"qinclude")
c	if(j2.ne.0) then
cc	write(6,*) "there is an include file"
cc there is a qinclude statement before the current file type is
cc determined. is this a command passer or a file opener?
c	j3=index(asci,line(j2+9:j2+9))
c	if(j3.eq.0) then
cc the qinclude statement does not open a file, but passes a
cc command, therefore pass that command
c	call namleb(line,j4)
c	call pi(line(j2+10:j4-2),j4-2-j2-9)
c	end if
c	if(j3.ne.0) then
cc	write(6,*) "it opens a file"
cc the include statement opens a file, therefore increment ifn
cc go back to 10 if it exists
c	call namleb(line,j4)
c	filnam=line(j2+9:j4-1)
c	ifillen=j4-j2-9
c	if(filnam.eq." ") then
c        call system("echo $HOME > tmp101")
c        open(7,file="tmp101")
c        read(7,'(a)')uhomenam
c        call namlen(uhomenam,uhomelen)
c	filnam=uhomenam(:uhomelen)//"/qpref.prm"
c	ifillen=uhomelen+10
c	write(6,*) "opening default file qpref.prm"
c        close (7)
c        call system("/bin/rm tmp101")
c	end if
c	inquire(file=filnam(:ifillen),exist=iext)
c	if(iext) then
c	ifn=ifn+1
c	write(6,*) "opening file: ",filnam(:ifillen)
c	open(ifn,file=filnam(:ifillen))
c	goto 10
c	end if
c	if(.not.iext) then
c	write(6,*) "the file ",filnam(:ifillen)," specified in"
c	write(6,*) "a qinclude statement does not exist. continuing"
c	end if
c	end if
c	end if
cc end of dealing with qinclude
cc
c	if(j2.eq.0) then
cc no qinclude, therefore define this as a type 2 parameter file
cc deal with possible multiply lines, deal with errors or
cc errroneous end of files
c	itype(ifn)=2
c	iblen=1
c30	call namleb(line,ilen)
cc	write(6,*) "passing first line",line(:ilen)
c	if(line(ilen:ilen).eq."&") then
c	mline(iblen:iblen+ilen-2)=line(:ilen-1)
c	iblen=iblen+ilen-1
c	read(ifn,204,err=400,end=300) line
c	goto 30
c	else
c	mline(iblen:iblen+ilen-1)=line(:ilen)
c	end if	
c	call pi(mline,ilen)
c	goto 302
c400	write(6,*) "error reading file :"
c	write(6,*) filnam
c	write(6,*) "continuing.."
c	goto 100
c300	write(6,*) "error, continuation slash on last line of"
c	write(6,*) "file ",filnam
c	write(6,*) "untoward things may happen"
c	goto 100
c302	continue
c	end if
c	end if
cc
c	if(index(cnumb,line(1:1)).ne.0) then
cc the firstcharacter is a number , therefore define this is as a type 1
cc without a qinclude on line 1
c	itype(ifn)=1
c	call prm1(line,lin1(ifn))
c	lin1(ifn)=lin1(ifn)+1
c	end if
cc
cc leave, having determined ttype unless a qinclude was encountered
cc
c	goto 450
c	end if
cc
cc type is already determined
cc
cc ! need to check for qincludes! different for type 1 and type 2
cc
c	j1=index(line,"QINCLUDE")
c	j2=j1+index(line,"qinclude")
c	if(j2.ne.0) then
cc	write(6,*) "include statement found"
cc there is a qinclude statement
cc determined. is this a command passer or a file opener?
c	j3=index(asci,line(j2+9:j2+9))
c	if(j3.eq.0) then
cc the qinclude statement does not open a file, but passes a
...
c	end if
cc
cc end of opening fil "if",j3
c	end if
c	goto 450
c	end if
cc
cc
cc if no qincludes continue as below
c	if(itype(ifn).eq.1) then
c	call prm1(line,lin1(ifn))
c	lin1(ifn)=lin1(ifn)+1
c	end if
cc
c	if(itype(ifn).eq.2) then
c	iblen=1
c31	call namleb(line,ilen)
cc
c	if(line(ilen:ilen).eq."&") then
c	mline(iblen:iblen+ilen-2)=line(:ilen-1)
c	iblen=iblen+ilen-1
c	read(ifn,204,err=401,end=301) line
c	goto 31
c	else
c	mline(iblen:iblen+ilen-1)=line(:ilen)
c	end if	
...
c	goto 303
cc
c401	write(6,*) "error reading file :"
c	write(6,*) filnam
c	write(6,*) "closing file and continuing.."
c	goto 100
c301	write(6,*) "error, continuation slash on last line of"
c	write(6,*) "file ",filnam
c	write(6,*) "which is now closed. untoward things may happen"
c	goto 100
c303	continue
c	end if
cc
...
c	goto 20
cc
cc deal with end of file
c500	write(6,*) "trouble at mill"
c100	close(ifn)
cc deal with end of file
c	if(ifn.gt.10) then
c	ifn=ifn-1
...
c
c	if(.not.ipdbrd) then
c 	open(13,file=pdbnam(:pdblen),form='formatted')
c        read(13,204,iostat=n)line
c	ias=0
c	do 600 i=1,80
c	if(index(asci,line(i:i)).eq.0) ias=ias+1
c600	continue
c	if(ias.gt.10) ipdbrd=.true.
c 	close(13)
c	end if
cc
c	if(ifrcwrt) then
c 	open(15,form='formatted')
c        read(13,204,iostat=n)line
c	ias=0
c	do 610 i=1,80
c	if(index(asci,line(i:i)).eq.0) ias=ias+1
c610	continue
c	if(ias.gt.10) ifrcrd=.true.
c 	close(15)
c	end if
c
...
c
	...
	ieq1=ieq-1
c       ieq1 = length of what is before the = sign
c
	ieq2=lineln-ieq
	type(1:ieq1)=line(1:ieq1)
