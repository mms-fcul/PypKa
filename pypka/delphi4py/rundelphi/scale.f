...
c (S. Sridharan         May 1994)
	subroutine sclbp(natom,igrid,xn1,scale,radprb,oldmid,ibnum,
     &	extot,iall,vcrd,vnr,nobject,numbmol,ibnumsurf,realsiz)
	include 'acc2.h'
...
c b+++++++++++++++++++++++++++++++++++++++++
        ...
        integer ibnumsurf,epsdim,imezzo(6),iente(6),ix,iy,iz,realsiz
        integer iepsmp(igrid,igrid,igrid,3),atsurf(ibnum),iac1,iac2
        ...
        real radius,modul(3),vectz(3),mod2(3),axdist,temp
        real vecty(3),xq(3),alpha,tan2,dot(3),dx,dy,dz
        real dix,diy,diz,hgs,ds2min1,ds2min2
        i_internal=memalloc(i_internal,1,nobject)
...
c  iflag sarà vero se il bgp è interno ma non con un oggetto
            do ii=2,6
...
c non sono sicurissimo perche' non ricordo esattmente che fa poi con prevmed...
            if (iflag) then
              ...
              dx2=xg2-xn1(2,ia)
              dx3=xg3-xn1(3,ia)
              dis2=dx1*dx1+dx2*dx2+dx3*dx3-rad3(ia)**2
c dis2, and ds2min are distance**2 from center - radius**2 so they can be <0
              if (dis2.lt.ds2min1) then
                iac2=iac1
	          ds2min2=ds2min1
                iac1=ia
	          ds2min1=dis2
	        elseif(dis2.le.ds2min2) then 
                iac2=ia
	          ds2min2=dis2
	        endif
            else
     ...
     &        'Problems in Scaling multidielectric Boundary Grid Points'
          ...
          atndx(i)=-1
c looks like atndx is used to build Delunay surface, so excluding these bgps
   	    dx1=xn1(1,iac2)-xn1(1,iac1)
          dx2=xn1(2,iac2)-xn1(2,iac1)
          dx3=xn1(3,iac2)-xn1(3,iac1)
          temp=dx1*dx1+dx2*dx2+dx3*dx3
          temp=0.5*(ds2min2-ds2min1)/temp
          vcrd(1,i)=xg1+temp*dx1
          vcrd(2,i)=xg2+temp*dx2
          vcrd(3,i)=xg3+temp*dx3
          vnr(1,i)=0.0
          vnr(2,i)=0.0
          vnr(3,i)=0.0
          go to 500
...
c I want to know if u is within the shell sorrounding the object
     ...
     &  .true.,zeta,axdist)
                 if (dist.gt.0.0.and.dist.lt.radprb(1)-1.e-6)out=.false.
                endif
c e+++++++++++++++++++++++++++++
              end if
	    end do
...
c b++++++++++++++++++++++++++++
        i_internal=memalloc(i_internal,0,0)
c e++++++++++++++++++++++++++++
c	 do i=1,ibnum
