...
123 	continue
	...
	i_vindx = memalloc(i_vindx,intsiz,3*mxtri)
	i_vert = memalloc(i_vert,realsiz,3*mxvtx)
...
c scale boundary grid point positions relative to acc data
	...
	i_vnorm = memalloc(i_vnorm,realsiz,3*vtot)
	i_vnorm2=0
	i_vnorm2 = memalloc(i_vnorm2,realsiz,3*vtot)
        call ddtime(tary)
        call sclbp(natom,igrid,xn1,scale,radprb,oldmid,vtot,
     &  extot,iall,vert,vnorm,nobject,numbmol,ibnumsurf,realsiz)
...
c hole-fixing variables
	...
	i_vtlst = memalloc(i_vtlst,intsiz,6*imxvtx)
	i_tmlst = memalloc(i_tmlst,intsiz,9*imxtri)
	i_vtpnt = memalloc(i_vtpnt,intsiz,imxvtx)
...
c	    write(*,*) "number of triangles added to fix holes= ",ntot2
	...
	i_vtemp = memalloc(i_vtemp,0,0)

	i_vtlen=memalloc(i_vtlen,0,0)
	i_vtlst=memalloc(i_vtlst,0,0)
	i_tmlst=memalloc(i_tmlst,0,0)
	i_vtpnt=memalloc(i_vtpnt,0,0)
