...
c tmpmap1(igrid,igrid,igrid),radius,cost,rad3(natom)
	  ...
	  integer inearest(6),iext,ibgp,cont
...
c b+++++++++++++++++++++++
c       i_tmpmap=memalloc(i_tmpmap,realsiz*igrid*igrid*igrid)
c       i_gchrgtmp=memalloc(i_gchrgtmp,realsiz*ngcrg)
...
821	continue
        ...
        i_gchrg=memalloc(i_gchrg,realsiz,n)
        i_gchrgd=memalloc(i_gchrgd,realsiz,n)
        i_gchrg2=memalloc(i_gchrg2,4,n)
        i_qval=memalloc(i_qval,realsiz,n)
        i_iqpos=memalloc(i_iqpos,4,n)
        i_gval=memalloc(i_gval,realsiz,n)
c
...
100	continue 
	icount1b=n
	    ...
	    write(6,*)nqgrd,nqass,icount1a,icount1b,nmedia,n,igrid
...
1821    continue
        ...
        i_gchrgtmp=memalloc(i_gchrgtmp,realsiz,icount1b)
...
c b++++++++++++++++++++
        iext=0
        ibgp=0
        inearest(1)=iepsmp(ix,iy,iz,1)/epsdim
        inearest(2)=iepsmp(ix,iy,iz,2)/epsdim
        inearest(3)=iepsmp(ix,iy,iz,3)/epsdim
        inearest(4)=iepsmp(ix-1,iy,iz,1)/epsdim
        inearest(5)=iepsmp(ix,iy-1,iz,2)/epsdim
        inearest(6)=iepsmp(ix,iy,iz-1,3)/epsdim
...
400	continue 
        i_gchrgtmp=memalloc(i_gchrgtmp,0,0)
        ibc=ib
        i_cgbp=memalloc(i_cgbp,realsiz,5*ibc)
	if(verbose)
     &  write(6,*) 'no. charged boundary grid points =',ibc
	 if (ibc.gt.ibcmax) then
	   write(6,*)'ibcmax parameter must be increased at least to',ibc
	   write(6,*)'Exiting'
	   stop
	 endif
	if(ico.ne.0) then
          write(6,*) "## out of them,",ico," charged points 
     &are in solution ## Increased resolution is needed"
c          write(6,*)'Exiting'
c          stop !LinLi commend the stop
        end if
c
...
c 
	do 500 i=1,icount1b
	  ...
	  gval(j)=gchrg(i)
500	continue 
        i_gchrgd=memalloc(i_gchrgd,0,0) 
c
c make iqpos
c 
	isgrid=igrid**2
	do 600 i=1,icount1b
	  ...
	  iqpos(j)=iv
600	continue 
        i_gchrg2=memalloc(i_gchrg2,0,0)
c
