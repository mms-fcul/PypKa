...
c (S. Sridharan		May 1994)
	subroutine vwtms(ibnum,xn1,natom,oldmid,limeps,nmedia
...
c b+++++++++++++
	integer bndeps(igrid,igrid,igrid,2),eps(6),nt,
...
c e+++++++++++++
	...
	i_r0= memalloc(i_r0,realsiz,natom)
	i_r02= memalloc(i_r02,realsiz,natom)
	i_rs2= memalloc(i_rs2,realsiz,natom)
...
c      if (.true.) then
c momentarily debug 
c          open(52,file='epsmap_before.txt',form='formatted')
c          do iiix=1,22
c	      do iiiy=1,22
c            write(52,'(3f8.2,i3)') (-11.5+iiix),(iiiy-12.0),0.0,
c     &            iepsmp(iiix,iiiy,11,1)
c            write(52,'(3f8.2,i3)') (iiix-12.0),(-11.5+iiiy),0.0,
c     &            iepsmp(iiix,iiiy,11,2)
c	      end do
c          end do
c      end if
...
c debugging++++++++++++++++++++++++++++++
          if (debug) then
            nt=0
c passed from mod to dim, I need the medium
...
c e++++++++++++++++++++++++++++++++++++++
     ...
     &      'reading accessible surface arcs data from file  ARCDAT'
	    extot=nacc
	    i_expos= memalloc(i_expos,realsiz,3*extot)
...
c temporary
c         extot=nacc
c         i_expos= memalloc(i_expos,realsiz,3*extot)
...
c         goto 111
      ...
      call ddtime(tary)
	call sas(xn1,natom,radprb,extot,nobject,numbmol,scale,realsiz)
      call ddtime(tary)
      if (verbose) write(6,*)'mkacc time = ',tary(1)
...
c debug++++++++++++++++++++++++
      if (debug) then
        ...
        do iiii=1,extot
          write (52,*) expos(1,iiii),expos(2,iiii),expos(3,iiii)
        end do
        close (52)
      end if
c end debug+++++++++++++++++++
...
c link the accessible points into iab1 and iab2
	...
	cba=1./grdi
	 ...
	 if(verbose)write(6,*)'grid for indexing accessible points = ',cba
...
c b++++++++++++++++++++++++++++++++++++
c remov is true if it is an internal midpoint close to an interface where a
...
c tengo il mod perche'' deve prendere solo punti in atomi
	...
	eps(2)=mod(iepsmp(ix,iy,iz,2),epsdim)
      remov=((eps(1).gt.1.and.eps(1).le.natom+1).or.(eps(2).gt.1.and.
     &  eps(2).le.natom+1))
	eps(3)=mod(iepsmp(ix,iy,iz,3),epsdim)
	eps(4)=mod(iepsmp(ix-1,iy,iz,1),epsdim)
      remov=((eps(3).gt.1.and.eps(3).le.natom+1).or.(eps(4).gt.1.and.
     &  eps(4).le.natom+1)).or.remov
	eps(5)=mod(iepsmp(ix,iy-1,iz,2),epsdim)
	eps(6)=mod(iepsmp(ix,iy,iz-1,3),epsdim)
      remov=((eps(5).gt.1.and.eps(5).le.natom+1).or.(eps(6).gt.1.and.
     &  eps(6).le.natom+1)).or.remov
...
cWWW     there is still an issue in case there are both molecules and objects: since parent object of reentrant points is only known
c in  sclbp, filling reentrant regions due to molecules in objects might fail
...
c       essere in poro ==> eps2=0 and eps >0
        if(eps(j).eq.0.or.(remov.and.eps(j).gt.natom+1).or.
...
c dis= distance to atom surface
	...
	if(iac.eq.0)then
         if (debug) then
	      write(6,*)"bgp:",i," might be a cavity point",ix,iy,iz
	      write(6,*)"midpoint",j," in position [A]"
     &                                  ,s1,s2,s3
	      write(6,*)"it1:",it1," it2:",it2," it3:",it3
	   end if
	  ncav=ncav+1
c possibly a cavity point
	  goto 201
	else
c check to see if it is in the contact region of that atom or object by projecting it to
...
c here and to improve this choice!WWW
     ...
     &+imedia*epsdim
        eps2(j)=imedia
c      not assigning the owner but only the medium, the former will be assigned in the scale routine
c e+++++++++++++++++++++++
...
c in other words: midpoint must remain external to objects
c b+++++++++++++
            do jj=1,6
c             in this way I can deal with eps(jj)<0
              isign=1
c             iord=owner of the midpoint jj before change or after eps=1
              iord=mod(iepsmp(ix+imap(1,jj),iy+imap(2,jj),iz+
     &             imap(3,jj),imap(4,jj)),epsdim)
c             the last changed for sure has not iord<0
c             there can be iord<0 due to nearest neighbors already changed
              if (iord.lt.0) goto 202
c             if it has changed at previous step, dont change anymore
              if(eps(jj).lt.0) then
                isign=-1
                if (iord.eq.0) iord=1
              end if
...
c             left  iord definition with mod since if it is <> 0, it keeps its identity
202           continue
            end do
...
c
	n1=n2+1
	n2=n2+m
	if(verbose) write(6,*)'bgp added m=',m,' bgp removed  mr =',mr
        if(m.gt.mpr)then
          ndv=ndv+1
c         if(ndv.gt.2)then
c Lin Li: the value used to be 2, seems not enough for some problem.
          if(ndv.gt.20)then
	...
	i_cbn1= memalloc(i_cbn1,0,0)
	i_cbn2= memalloc(i_cbn2,0,0)
	i_cbal= memalloc(i_cbal,0,0)
...
c debug++++++++++++++++++++++++
	...
	write(52,*)igrid,ibnum
	      ...
	      iy= ibgrd(3*iiii-1)
 	      iz= ibgrd(3*iiii)
	      iext=bndeps(ix,iy,iz,2)
c old           write(52,*)iiii,ibnd(3*iiii-2),ibnd(3*iiii-1),ibnd(3*iiii)
            write(52,*)iiii,ix,iy,iz,iext
            if(bndeps(ix,iy,iz,1).eq.0) write(52,*)"problems",iiii
          end do
        close (52)
	endif
c end debug+++++++++++++++++++

        i_bndeps=memalloc(i_bndeps,0,0)
        i_ibnd= memalloc(i_ibnd,0,0)
c
c scale bondary grid point positions relative to acc data
	    ...
	    i_scspos= memalloc(i_scspos,realsiz,3*ibnum)
	    do j=1,ibnum
	      j3=3*j
	      scspos(j3-2)=float(ibgrd(j3-2))
  	      scspos(j3-1)=float(ibgrd(j3-1))
  	      scspos(j3)=float(ibgrd(j3))
	    end do
c b++++++++++
          i_scsnor=memalloc(i_scsnor,realsiz,3*ibnum)
	    i_atsurf= memalloc(i_atsurf,4,ibnum)
	    i_atndx = memalloc(i_atndx,4,ibnum)
	    call sclbp(natom,igrid,xn1,scale,radprb,oldmid,ibnum,
     &	   extot,iall,scspos,scsnor,nobject,numbmol,ibnumsurf,realsiz)
	    call ddtime(tary)
	    if (verbose) then
     ...
     &         ' points had to be assigned by global comparison'
          ...
          if(isrf.and.ivertnorm)then
            open(52,file='surf.xyzn',form='formatted')
c           real coordinates
            do iiii=1,ibnum
	        write(52,'(6f10.2)')scspos(3*iiii-2),scspos(3*iiii-1),
     &        scspos(3*iiii),scsnor(3*iiii-2),scsnor(3*iiii-1),
     &        scsnor(3*iiii)
            end do
            close (52)
		endif
          if (.not.isite) i_scsnor=memalloc(i_scsnor,0,0)
        endif
     ...
     &           egrid,nobject,numbmol,ibnumsurf)
            i_egrid= memalloc(i_egrid,0,0)
          else
            write(6,*)'msrf routine cannot be run'
            write(6,*)'because there are also geometric objects'
          endif
        endif
        if(.not.isitsf.and..not.isite.and..not.(isch.and.scrgfrm.ne.0))
     &    then
	    i_atndx = memalloc(i_atndx,0,0)
          i_atsurf= memalloc(i_atsurf,0,0)
	  end if
c e++++++++++
        i_iab1= memalloc(i_iab1,0,0)
        i_iab2= memalloc(i_iab2,0,0)
        i_icume= memalloc(i_icume,0,0)

	i_r0= memalloc(i_r0,0,0)
	i_r02= memalloc(i_r02,0,0)
	i_rs2= memalloc(i_rs2,0,0)
	i_ast= memalloc(i_ast,0,0)
