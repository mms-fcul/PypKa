...
c
	...
	dimension ibndx(nbgp),ibndy(nbgp),ibndz(nbgp)
	character*24 day
	character*1 symb
...
c               if ((der.gt.0.and.conv(1).gt..1).and..not.inew.and..not.
     &inewfirst) then
                  ichangeom=.true.
              factor=min((factor*.05/conv(1)),factor*(1.-der)**.86)
                end if
                if (der.le.0.) then
                   icountplus=0
...
c            if(temp3.gt.2500) qmap1(ix)=0.
c            if(temp4.gt.2500) qmap2(ix)=0.
c 2012-04-24 Chuan set cutoff values of qmap1 and qmap2 nonzero
...
c 2012-04-24 Chuan set cutoff values of qmap1 and qmap2 nonzero
            if (temp3.gt.2500) then
               qmap1(ix)=fac1*2500.*(chi2+50.*(chi3+50.*(chi4+50.*chi5)))
            end if             
            if (temp4.gt.2500) then 
               qmap2(ix)=fac1*2500.*(chi2+50.*(chi3+50.*(chi4+50.*chi5)))
            end if
c e+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
