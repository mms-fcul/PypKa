...
class Tautomer(object):
    ...
    def loadChargeSet(self, res_name, ref_tautomer):
        ...
        fname = '{0}/{1}/sts/{2}tau{3}.st'.format(Config.pypka_params['script_dir'],
    ...
    def setDetailsFromWholeMolecule(self):
        ...
        natoms = delphimol.changeStructureSize(p_atpos, p_rad3,
                                               p_chrgv4, atinf,
                                               p_iatmed,
                                               extra_atoms=new_atoms, natoms=delphimol.natoms)
        ...
        if Config.debug:
            ...
            with open(filename, 'w') as f_new:
                ...
                pdb_text += new_pdb_line(-3, 'P', 'PDB', -3,
                                         box[0] - cutoff, cutoff, z)
                pdb_text += new_pdb_line(-3, 'P', 'PDB', -3,
                                         cutoff, box[1] - cutoff, z)
            ...
            with open('P_{1}-{0}.crg'.format(self.name, self.site.res_number), 'w') as f_new:
                f_new.write(crg)
    ...
    def setDetailsFromTautomer(self):
        ...
        delphimol.changeStructureSize(p_atpos, p_rad3,
    ...
    def CalcPotentialTautomer(self):
        ...
        logfile = 'LOG_runDelPhi_{0}_{1}_modelcompound'.format(self.name,
                                                               self.site.res_number)
        if Config.debug:
    ...
    def CalcPotentialTitratingMolecule(self):
        ...
        filename = '{0}_{1}.prm'.format(self.name, self.site.res_number)
        logfile = 'LOG_runDelPhi_{0}_{1}_wholeprotein'.format(self.name,
                                                              self.site.res_number)
        ...
        if Config.delphi_params['pbc_dim'] == 2:
            delphimol.runDelPhi(scale_prefocus=Config.delphi_params['scaleP'],
                                ...
                                filename=filename,
                                outputfile=logfile)
        elif Config.delphi_params['pbc_dim'] == 0 and Config.delphi_params['bndcon'] == 3:
            delphimol.runDelPhi(scale_prefocus=Config.delphi_params['scaleP'],
                                scale=Config.delphi_params['scaleM'],
                                nlit_prefocus=Config.delphi_params['nlit'],
                                nonit=0,
        ...
        if Config.debug:
            filename = '{0}_{1}.frc'.format(self.name,
                                            self.site.res_number)
    ...
    def calcBackEnergy(self):
        ...
        for atom_position in range(delphimol.natoms):
            ...
            if atom_id == None or atom_id not in self.site.getAtomNumbersList():
                ...
                if cutoff != -1 or distance <= cutoff2:
                    point_energy = delphimol.p_chrgv4[atom_position] * self.sitpotM[atom_position]
        ...
        if Config.debug:
            filename = '{0}_{1}_eback.xvg'.format(self.name,
                                                  self.site.res_number)
            with open(filename, 'w') as f_new:
