...
class AnyTestCase(unittest.TestCase):
    ...
    def check_parse_empty_all(self):
        ...
        for typeclass in filter(
            lambda c: type(c) in [types.ClassType, type]
            and not issubclass(c, TC.String)
            and issubclass(c, TC.SimpleType),
            TC.__dict__.values(),
        ):
            if typeclass in skip:
                continue
            tc = typeclass()
            sw = SoapWriter()
    ...
    def check_builtins(self):
        myInt, myLong, myStr, myDate, myFloat = (
            123,
            2147483648,
            "hello",
            time.gmtime(),
...
<tns:foo %(tns)s %(xsi)s %(soap)s>
    ...
    def check_any_typed_soap_integer(self):
        ...
        d.update(NSDICT)
        xml = (
            """<tns:i xsi:type="SOAP-ENC:integer" %(xsi)s %(soap)s %(tns)s>%(value)d</tns:i>"""
            % d
        )
        ps = ParsedSoap(xml, envelope=False)
    ...
    def check_any_typed_xsd_int(self):
        ...
        d.update(NSDICT)
        xml = (
            """<tns:i xsi:type="xsd:int" %(xsi)s %(soap)s %(tns)s %(xsd)s>%(value)d</tns:i>"""
            % d
        )
        ps = ParsedSoap(xml, envelope=False)
        self.failUnless(ps.Parse(TC.Any()) == value)
    ...
    def check_any_typed_nonNegativeInteger(self):
        ...
        d.update(NSDICT)
        xml = (
            """<tns:i xsi:type="xsd:nonNegativeInteger" %(xsi)s %(soap)s %(tns)s %(xsd)s>%(value)d</tns:i>"""
            % d
        )
        ps = ParsedSoap(xml, envelope=False)
        self.failUnless(ps.Parse(TC.Any()) == value)
...
for t in [
    i[0].split(_SEP)
    for i in filter(lambda i: callable(i[1]), AnyTestCase.__dict__.items())
]:
    test = ""
    for f in t:
        test += f
        if globals().has_key(test):
            test += _SEP
            continue
