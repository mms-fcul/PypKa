...
class t1TestCase(unittest.TestCase):
    ...
    def checkt1(self):
        ...
        self.failUnlessEqual(TC.Integer(None, nillable=True).parse(elts[10], ps), None)
        self.failUnlessEqual(TC.Ibyte(None, nillable=True).parse(elts[10], ps), None)
        B = [TC.Integer("Price"), TC.Integer("p2"), TC.String(unique=1)]
        self.failUnlessEqual(TC.Integer(("test-uri", "Price")).parse(elts[0], ps), 34)
        ...
        self.failUnlessEqual(TC.String("Name").parse(elts[2], ps), u"This is the name")
        self.failUnlessEqual(TC.Any("Price").parse(elts[0], ps), 34)
        self.failUnlessEqual(TC.Any("n3").parse(elts[4], ps), u"The value of n3")
        TC.XML("n2").parse(elts[3], ps)
        nodelist = TC.XML("a2").parse(elts[7], ps)
        self.failUnlessEqual(TC.String("n3").parse(elts[4], ps), u"The value of n3")
        self.failUnlessEqual(TC.Base64String("n64").parse(elts[5], ps), u"hello")
        self.failUnlessEqual(TC.String("n64").parse(elts[5], ps), u"a GVsbG8=")
        enum = TC.Enumeration(["Red", "Blue", "Green"], "color")
        self.failUnlessEqual(enum.parse(elts[6], ps), u"Red")
        self.failUnlessEqual(TC.IEnumeration([44, 46, 47]).parse(elts[1], ps), 44)
        S = TC.Struct(None, [TC.String("t"), TC.Integer("i")], inorder=0)
        pyobj = S.parse(elts[8], ps)
        S2 = TC.Struct(
            myclass,
            [TC.IunsignedShort("i"), TC.String("q:z", minOccurs=0), TC.String("t")],
            "a2",
            typed=0,
        )
        pyobj2 = S2.parse(elts[8], ps)
        self.failUnlessEqual(TC.URI().parse(elts[12], ps), u'"http://foo.com/~salz"')
        ...
        self.failUnlessEqual(
            TC.Array(("test-uri", "x"), TC.Any()).parse(elts[15], ps),
            [u"The value of n3", u"rich salz", 13],
        )
        self.failUnlessEqual(
            TC.Struct(None, (TC.FPfloat("a"), TC.Decimal("b"), TC.FPdouble("c"))).parse(
                elts[13], ps
            ),
            {"a": 6.9000000000000004, "c": TC._make_inf(), "b": 0.0},
        )
        ...
        self.failUnlessEqual(
            TC.Any().parse(elts[11], ps), {"urt-i": 12, "urt-t": u"rich salz"}
        )
...
class myclass:
    ...
    def __str__(self):
        return 'myclass-%s-(%d,"%s")' % (self.name, self.i, self.t) + str(self.z)
