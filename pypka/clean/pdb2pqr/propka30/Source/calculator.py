...
def CoulombEnergy(distance, weight, version, options=None):
    ...
    if isinstance(version.coulomb_diel, list):
        diel = (
            version.coulomb_diel[1]
            - (version.coulomb_diel[1] - version.coulomb_diel[0]) * weight
        )
...
def distanceScaledCoulombEnergy(distance, weight, version, options=None):
    ...
    scale = (R - version.coulomb_cutoff[1]) / (
        version.coulomb_cutoff[0] - version.coulomb_cutoff[1]
    )
    scale = max(0.0, scale)
    scale = min(1.0, scale)
...
def MixedCoulombEnergy(distance, weight, version, options=None):
    ...
    dpka_sur = 244.12 / (80.0 * R) - 244.12 / (80.0 * version.coulomb_cutoff[1])
...
def radialVolumeDesolvation(residue, atoms, version, options=None):
    ...
    if residue.label == "BKB  50 A":
        pka_print(
            "found %s [%6.3lf%6.3lf%6.3lf]!"
            % (residue.label, residue.x, residue.y, residue.z)
        )
    ...
    for chainID in atoms.keys():
        for key in atoms[chainID]["keys"]:
            for atom in atoms[chainID][key]:
                if atom.element != "H":
                    if (
                        atom.resNumb != residue.resNumb
                        or atom.chainID != residue.chainID
                    ):
                        # selecting atom type
                        ...
                        if distance_sqr < version.desolv_cutoff_sqr:
                            dV_inc = dV / max(
                                min_distance_4th, distance_sqr * distance_sqr
                            )
                            volume += dV_inc
                            if residue.label in [
                                "ASP   8 a",
    ...
    scale_factor = 1.0 - (1.0 - version.desolvationSurfaceScalingFactor) * (
        1.0 - weight
    )
    ...
    residue.Emass = (
        residue.Q
        * version.desolvationPrefactor
        * max(0.00, volume - version.desolvationAllowance)
        * scale_factor
    )
...
def contactDesolvation(residue, atoms, version, options=None):
    ...
    residue.Emass = (
        residue.Q
        * version.desolvationPrefactor
        * max(0.00, residue.Nmass - version.desolvationAllowance)
    )
...
def originalDesolvation(residue=None, atoms=None, version=None, options=None):
    ...
    for chainID in atoms.keys():
        for key in atoms[chainID]["keys"]:
            for atom in atoms[chainID][key]:
                HYDROGEN_ATOM = (atom.name[0] == "H") or (
                    atom.name[0] in string.digits and atom.name[1] == "H"
                )
...
def BackBoneReorganization(protein):
    ...
    for residue in residues:
        ...
        for atom3, atom2 in protein.COlist:
            ...
            distance, f_angle, nada = AngleFactorX(
                atom2=atom2, atom3=atom3, center=center
            )
...
def bracketingPI(protein, bracket=[0.0, 14.0]):
    ...
    while True:
        ...
        iter += 1
        pka_print(
            "%4d protein = %6.2lf [%6.2lf%6.2lf] [%6.2lf%6.2lf]"
