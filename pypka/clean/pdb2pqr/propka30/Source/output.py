                ...
                for atom in residue.atoms:
                    ...
                    if distance > 16.0:
                        ...
                        if residue.label == label:
                            if (
                                residue.resName == "ASP" and atom.name in ["OD1", "OD2"]
                            ) or (
                                residue.resName == "GLU" and atom.name in ["OE1", "OE2"]
                            ):
                                line = "%s %6.2lf%6.2lf\n" % (
                                    pdbline[:55],
                                    -0.70,
                                ...
                                solute.write(line)
                            elif (residue.resName == "ASP" and atom.name in ["CG"]) or (
                                residue.resName == "GLU" and atom.name in ["CD"]
                            ):
                                line = "%s %6.2lf%6.2lf\n" % (
                                    pdbline[:55],
                                    0.40,
...
def writePKA(
    protein, filename=None, reference="neutral", direction="folding", options=None
):
    """
...
def printTmProfile(
    ...
    profile = protein.getTmProfile(
        reference=reference, grid=[0.0, 14.0, 0.1], Tms=Tms, ref=ref, options=options
    )
    if profile == None:
        ...
        for (pH, Tm) in profile:
            if (
                pH >= window[0]
                and pH <= window[1]
                and (pH % window[2] < 0.01 or pH % window[2] > 0.99 * window[2])
            ):
                str += "%6.2lf%10.2lf\n" % (pH, Tm)
        if verbose:
            print(str)
...
def getFoldingProfileSection(
    ...
    str += "\n"
    str += (
        "Free energy of %9s (kcal/mol) as a function of pH (using %s reference)\n"
        % (direction, reference)
    )

    profile = protein.getFoldingProfile(
        reference=reference, direction=direction, grid=[0.0, 14.0, 0.1], options=options
    )
    if profile == None:
    ...
    if pH == None or dG == None:
        ...
        str += (
            "The pH of optimum stability is %4.1lf for which the free energy is%6.1lf kcal/mol at 298K\n"
            % (pH, dG)
        )
    ...
    if dG_min == None or dG_max == None:
        str += (
            "Could not determine pH values where the free energy is within 80 %s of maximum\n"
            % ("%")
        )
    else:
...
def getChargeProfileSection(protein, verbose=False, options=None):
    ...
    profile = protein.getChargeProfile(grid=[0.0, 14.0, 1.0], options=options)
    if profile == None:
...
def writeWhatIfFile(
    mutationData=None, pdbfile=None, newfile=None, filename="whatif.sh", options=None
):
    """
    writing a scap file for, i.e., generating a mutated protein
    """
