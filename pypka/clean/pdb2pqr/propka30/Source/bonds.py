...
class bondmaker:
    def __init__(self):
        ...
        self.intra_residue_backbone_bonds = {
            "N": ["CA"],
            "CA": ["N", "C"],
            "C": ["CA", "O"],
            "O": ["C"],
                ...
                if chain.residues[i].resName == "CYS":
                    for j in range(0, len(chain.residues)):
                        if chain.residues[j].resName == "CYS" and j != i:
                            self.check_for_cysteine_bonds(
                                chain.residues[i], chain.residues[j]
                            )
        return
        ...
        for atom1 in residue1.atoms:
            if atom1.name == "C":
                for atom2 in residue2.atoms:
                    if atom2.name == "N":
                        if (
                            self.squared_distance(atom1, atom2)
    ...
    def find_bonds_for_residue_backbone(self, residue):
        for atom1 in residue.atoms:
            if atom1.name in list(
                self.number_of_pi_electrons_in_bonds_in_backbone.keys()
            ):
                atom1.number_of_pi_electrons_in_double_and_triple_bonds = (
                    self.number_of_pi_electrons_in_bonds_in_backbone[atom1.name]
                )
            if (
                atom1.name
                in list(
                    self.number_of_pi_electrons_in_conjugate_bonds_in_backbone.keys()
                )
                and len(atom1.bonded_atoms) > 1
            ):  # last part to avoid including N-term
                atom1.number_of_pi_electrons_in_conjugate_double_and_triple_bonds = (
    ...
    def find_bonds_for_side_chain(self, atoms):
        ...
        for atom1 in atoms:
            ...
            if key in list(self.number_of_pi_electrons_in_bonds_in_sidechains.keys()):
                atom1.number_of_pi_electrons_in_double_and_triple_bonds = (
                    self.number_of_pi_electrons_in_bonds_in_sidechains[key]
                )
            if key in list(
                self.number_of_pi_electrons_in_conjugate_bonds_in_sidechains.keys()
            ):
                atom1.number_of_pi_electrons_in_conjugate_double_and_triple_bonds = (
                    self.number_of_pi_electrons_in_conjugate_bonds_in_sidechains[key]
                )
        ...
        for atom in ligand.atoms:
            if atom.name in self.number_of_pi_electrons_in_bonds_ligands.keys():
                atom.number_of_pi_electrons_in_double_and_triple_bonds = (
                    self.number_of_pi_electrons_in_bonds_ligands[atom.name]
                )
            if (
                atom.name
                in self.number_of_pi_electrons_in_conjugate_bonds_in_ligands.keys()
            ):
                atom.number_of_pi_electrons_in_conjugate_double_and_triple_bonds = (
                    self.number_of_pi_electrons_in_conjugate_bonds_in_ligands[atom.name]
                )
            ...
            if atom.name in list(
                ...
                atom.number_of_pi_electrons_in_double_and_triple_bonds = (
                    self.number_of_pi_electrons_in_bonds_in_backbone[atom.name]
                )
            if (
                atom.name
                in list(
            ...
            for j in range(i + 1, len(atoms)):
                ...
                if sq_dist < self.SS_dist_squared:
                    if sq_dist < self.default_dist_squared:
                        ...
                        atoms[i].get_element() == "S" and atoms[j].get_element() == "S"
                    ):
    ...
    def generate_protein_bond_dictionary(self, atoms):
        ...
        for atom in atoms:
            for bonded_atom in atom.bonded_atoms:
                ...
                if (
                    not name_i in self.backbone_atoms
                    or not name_j in self.backbone_atoms
                ):
                    if (
                        not name_i in self.terminal_oxygen_names
...
if __name__ == "__main__":
    ...
    for chain in my_protein.chains:
        for residue in chain.residues:
            residue.atoms = [
                atom for atom in residue.atoms if atom.get_element() != "H"
            ]
