...
def readPDB(filename, file=None, verbose=True, tags=["ATOM"]):
    ...
    (
        ...
        models_lines,
    ) = scanFileForConfigurations(file)
    ...
    for current_model in models_lines:
        for line in current_model:
            ...
            if chainID not in atoms:
                pka_print(
                    "incorrect labeling in %s, please correct your pdbfile; could not find chain '%s'"
                    % (filename, chainID)
                )
                sys.exit(8)
            if reskey not in atoms[chainID]:
                pka_print(
                    "incorrect labeling in %s, please correct your pdbfile; could not find '%s'"
                    % (filename, reskey)
                )
                sys.exit(8)
...
def openPdbFile(filename):
    ...
    if extension == "pdb":
        ...
        pka_print(
            'trying to pass me a rotten pdbfile "%s" - no pdb extension' % (filename)
        )
...
class Atom:
    ...
    def setConfiguration(self, key=None):
        ...
        if key in self.configurations:
            ...
            for item in self.configurations.keys():
                keys += "%5s" % (item)
            pka_print(
                "configuration '%s' not found in '%s' atom '%s' [%s]"
                % (key, resLabel, self.name, keys)
            )
            sys.exit(8)
    ...
    def __str__(self):
        return "%5d-%4s %5d-%3s (%1s) [%8.3f %8.3f %8.3f]" % (
            self.numb,
            self.name,
            self.resNumb,
            self.resName,
