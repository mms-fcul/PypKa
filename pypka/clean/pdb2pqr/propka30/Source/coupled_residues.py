...
def is_coupled_pairwise(residue1, residue2):
    ...
    factor_diff_intrinsic_pka = get_pka_diff_factor(
        interaction_free_pka1, interaction_free_pka2
    )
...
def is_coupled_intrinsic_pka(residue1, residue2):
    ...
    interaction_energy = max(
        get_interaction(residue1, residue2), get_interaction(residue2, residue1)
    )
...
def is_coupled_protonation_state_probability(protein, residue1, residue2, options=None):
    ...
    if (
        max(default_pka1, default_pka2) < min_pka
        or min(default_pka1, default_pka2) > max_pka
    ):
        return {"coupling_factor": -1.0}
    ...
    if abs(default_energy - swapped_energy) <= max_free_energy_diff:
        if max(abs(pka_shift1), abs(pka_shift2)) >= min_swap_pka_shift:
            if (
                abs(residue1.intrinsic_pKa - residue2.intrinsic_pKa)
                <= max_intrinsic_pKa_diff
            ):
                factor = (
                    get_free_energy_diff_factor(default_energy, swapped_energy)
                    * get_pka_diff_factor(
                        residue1.intrinsic_pKa, residue2.intrinsic_pKa
                    )
...
def get_interaction_factor(interaction_energy):
    ...
    if interaction_energy >= min_interaction_energy:
        res = (interaction_energy - min_interaction_energy) / (
            1.0 + interaction_energy - min_interaction_energy
        )
...
def identify_coupled_residues(protein, options=None):
    ...
    if True:
        ...
        pka_print(
            "   Maximum pKa difference:     %4.2f pKa units" % max_intrinsic_pKa_diff
        )
        pka_print(
            "   Minimum interaction energy: %4.2f pKa units" % min_interaction_energy
        )
        pka_print(
            "   Maximum free energy diff.:  %4.2f pKa units" % max_free_energy_diff
        )
        ...
        for j in range(len(all_residues)):
            ...
            if do_intrinsic:
                (
                    ...
                    interaction_energy,
                ) = is_coupled_intrinsic_pka(all_residues[i], all_residues[j])
                if coupling_factor_intrinsic_pka > 0.0:
                    swap = 1
                    pka_print(
                        " %s and %s coupled (intrinsic pKa): %4.2f | intrinsic pkas: %5.2f - %5.2f = %5.2f | int. energy: %4.2f"
                        % (
                            all_residues[i],
                            all_residues[j],
                            coupling_factor_intrinsic_pka,
            ...
            if do_pair_wise:
                (
                    ...
                    int_free_pka2,
                    interaction_energy,
                ) = is_coupled_pairwise(all_residues[i], all_residues[j])
                if coupling_factor_pairwise > 0.0:
                    swap = 1
                    pka_print(
                        " %s and %s coupled (pair wise)    : %4.2f | int. free pkas: %5.2f - %5.2f = %5.2f | int. energy: %4.2f"
                        % (
                            all_residues[i],
                            all_residues[j],
                            coupling_factor_pairwise,
            ...
            if do_prot_stat:
                data = is_coupled_protonation_state_probability(
                    protein, all_residues[i], all_residues[j], options=options
                )
                if data["coupling_factor"] > 0.0:
                    ...
                    if verbose:
                        pka_print(
                            make_data_to_string(data, all_residues[i], all_residues[j])
                        )
...
def swap_interactions(residue1, residue2, include_side_chain_hbs=True, verbose=True):
    ...
    if verbose:
        ...
        tagged_pka_print(
            " Original|",
            residue1.getDeterminantString(),
            [residue1.label, residue2.label],
        )
        tagged_pka_print(
            " Original|",
            residue2.getDeterminantString(),
            [residue1.label, residue2.label],
        )

    # swap the interactions!
    transfer_determinant(
        residue1.determinants[2],
        residue2.determinants[2],
        residue1.label,
        residue2.label,
    )
    if include_side_chain_hbs:
        transfer_determinant(
            residue1.determinants[0],
            residue2.determinants[0],
            residue1.label,
            residue2.label,
        )
