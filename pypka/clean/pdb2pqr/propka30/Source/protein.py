...
class Protein:
    ...
    def __init__(self, atoms=None, pdbfile=None, name=None, options=None):
        ...
        self.property = (
            {}
        )  # a dictionary that will hold protein properties such as e.g. pI, folding-profile etc.
        self.configurations = (
            None  # a list of configuration keys valid for this protein.
        )
        self.propka_residues = (
            []
        )  # residues needed by propka, Have to be in resNumb order to break the pair-loop correctly
        self.NHlist = []
        self.COlist = []
        ...
        self.status = {"protonated": False, "done pka": False}
        ...
        for chainID in sorted(self.atoms.keys()):
            ...
            if options.chains == None or chainID in options.chains:
                self.chains.append(
                    Chain(self.atoms[chainID], resInfo=resInfo, options=options)
                )
            else:
        ...
        for residue in pka_residues:
            ...
            for type in range(3):
                for key in determinants[residue_key][type].keys():
                    ...
                    if (
                        abs(value) > 0.005
                    ):  # <-- removing determinant that appears as 0.00
    ...
    def calculateConfigurationPKA(self, version=None, options=None):
        ...
        determinants.setDeterminants(
            self.propka_residues, version=version, options=options
        )
    ...
    def writePKA(
        self, filename=None, reference="neutral", direction="folding", options=None
    ):
        """
    ...
    def getPHopt(
        ...
        if "pH-opt" in self.property:
            ...
            profile = self.getFoldingProfile(
                reference=reference, direction=direction, grid=grid, options=options
            )
            dG_opt = 999.0
    ...
    def getTmProfile(
        ...
        if "Tm-profile" not in self.property:
            if False:
                ...
                self.property["Tm-profile"] = calculate.TmProfile(
                    self,
                    reference=reference,
                    grid=grid,
                    Tm=Tm,
    ...
    def getChargeProfile(self, grid=[0.0, 14.0, 1.0], options=None):
        """
    ...
    def makeMutant(self, mutation=None, atoms=None, version=None, options=None):
        ...
        newProtein = mutate.makeMutatedProtein(
            self, mutation=mutation, atoms=atoms, options=options
        )

        return newProtein

    def optimizeMutationDeterminants(
        self, mutation=None, atoms=None, alignment=None, version=None, options=None
    ):
        """
        ...
        best_mutation = mutate.optimizeMutationDeterminants(
            self,
            mutation=mutation,
            atoms=atoms,
            alignment=alignment,
    ...
    def optimizeMultipleMutations(
        self, mutations=None, atoms=None, alignment=None, version=None, options=None
    ):
        """
        permutes multiple mutations and determins the most stable combination; note, you need the version for stability calculations
        """
        ...
        best_mutation = mutate.optimizeMultipleMutations(
            self,
            mutations=mutations,
            atoms=atoms,
            alignment=None,
    ...
    def calculateTitrationCurve(self, label=None, grid=[0.0, 14.0, 0.1], options=None):
        """
        Calculates the titration curve of residue 'label'
        """
        if label == None:
            pka_print(
                "Must specify residue label, cannot calculate titration curve on whole protein"
            )
            sys.exit(8)
        else:
    ...
    def calculateFoldingEnergy(self, pH=None, reference=None, options=None):
        ...
        dG = 0.00
        for chain in self.chains:
            dG += chain.calculateFoldingEnergy(
                pH=pH, reference=reference, options=options
            )

        return dG

    def compareWithExperiment(
        self,
        file,
        list,
        set=None,
...
def getResidueParameters():
    ...
    resInfo["Q"] = getQs()
    resInfo["pKa"] = pKa_mod()
    resInfo["type"] = {"C- ": "C-terminus", "N+ ": "N-terminus"}
