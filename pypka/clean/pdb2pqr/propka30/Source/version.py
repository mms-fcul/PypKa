...
def checkCooArgException(residue_coo, residue_arg, version=None):
    ...
    exception = True
    value_tot = 0.00
    dpka_max, cutoff = version.SideChainParameters[residue_coo.resType][
        residue_arg.resType
    ]
...
def checkCooCooException(residue1, residue2, version=None):
    ...
    for atom1 in residue1.makeDeterminantAtomList(residue2.resName, version=version):
        for atom2 in residue2.makeDeterminantAtomList(
            residue1.resName, version=version
        ):
...
class Version(object):
    ...
    def setBackBoneMaxPKA(self, dpka=None, resType=None):
        ...
        if resType == None:
            ...
            for key in self.BackBoneParameters.keys():
                ...
                str += "[%5.2lf,%5.2lf]" % (
                    self.BackBoneParameters[key][1][0],
                    self.BackBoneParameters[key][1][1],
                )
                # pka_print(str)
        else:
        ...
        if resType == None:
            ...
            for key1 in self.SideChainParameters.keys():
                ...
                for key2 in self.SideChainParameters[key1].keys():
                    ...
                    str = " %s: %6.2lf " % (
                        key2,
                        self.SideChainParameters[key1][key2][0],
                    )
                    str += "[%5.2lf,%5.2lf]" % (
                        self.SideChainParameters[key1][key2][1][0],
                        self.SideChainParameters[key1][key2][1][1],
                    )
                    # pka_print(str)
        else:
    ...
    def setCoulomb(
        self, label, max_dpka=None, cutoff=None, diel=None, scaled=None, mixed=False
    ):
        """
    ...
    def calculateSideChainEnergy(self, distance, dpka_max, cutoff, weight, f_angle):
        ...
        if False:
            ...
            prefactor = 1.00
        return calculate.HydrogenBondEnergy(
    ...
    def calculateCoulombEnergy(self, distance, weight, options=None):
        ...
        """
        if self.CoulombModel == "Linear":
            return calculate.linearCoulombEnergy(
                distance, weight, self, options=options
            )
            ...
            return calculate.distanceScaledCoulombEnergy(
                distance, weight, self, options=options
            )
        else:
...
class Jan01(Version):
    ...
    def __init__(self, options=None):
        ...
        self.SideChainParameters = parameters.getHydrogenBondParameters(
            type="side-chain"
        )
        self.interaction = getSideChainInteractionMatrix(
            side_chain=parameters.getInteraction()
        )  # interaction rule matrix ['N'/'I'/'-']
...
class Oct13(Version):
    ...
    def __init__(self, options=None):
        ...
        self.desolv_cutoff_sqr = self.desolv_cutoff * self.desolv_cutoff
        self.setDesolvation(
            "VolumeModel", prefactor=-13.12, surface=0.40, allowance=0.0
        )
...
class Oct14(Version):
    ...
    def __init__(self, options=None):
        ...
        self.setCoulomb(
            "Coulomb", cutoff=[3.5, 10.0], diel=80.0, scaled=False, mixed=True
        )
        self.doingBackBoneReorganization = True
        self.BackBoneParameters = parameters.getHydrogenBondParameters(type="back-bone")
        self.SideChainParameters = parameters.getHydrogenBondParameters(
