...
class old_scheme:
            ...
            for residue in chain.residues:
                if residue.type == "amino-acid":
                    ...
                    if residue.resType == "AMD":
                        ...
                        pka_print(
                            "no protocol to protonate '%s' in 'old-scheme'"
                            % (residue.label)
                        )
                        sys.exit(8)
    ...
    def protonateBackBone(self, residue, C, O):
        ...
        if C == None and O == None:
            ...
            pka_print(
                "could not find N atom in '%s' (protonateBackBone())" % (residue.label)
            )
            sys.exit(9)
...
class new_scheme:
    ...
    def trigonal(self, atom):
        ...
        if len(atom.bonded_atoms) == 1 and atom.number_of_protons_to_add > 0:
            ...
            if (
                atom.bonded_atoms[0].steric_number == 3
    ...
    def set_bond_distance(self, a, element):
        ...
        if element in list(self.bond_lengths.keys()):
            ...
            pka_print(
                "WARNING: Bond length for %s not found, using the standard value of %f"
                % (element, d)
            )
