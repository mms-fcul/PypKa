...
def compareFoldingContributions(target=None, template=None, options=None):
    ...
    alignment = readAlignmentFiles(
        filenames=options.alignment, mesophile=target.name, options=options
    )
    ...
    positions = makeFoldingEnergyDifferences(
        target, template, alignment, names=names, options=options
    )

    # print out contribution differences
    printFoldingEnergyDifferences(
        positions, names=names, template=template, options=options
    )
...
def checkDonePKA(target=None, template=None, alignment=None, options=None):
    ...
    if target.status["done pka"] == False:
        pka_print(
            "please calculate pKa values for target %s before comparing" % (target.name)
        )
    if template.status["done pka"] == False:
        pka_print(
            "please calculate pKa values for template %s before comparing"
            % (template.name)
        )
    ...
    if target.status["done pka"] == True and template.status["done pka"] == True:
        if False:
            pka_print(
                "target = %s, template = %s : pKas done" % (target.name, template.name)
            )
    else:
        sys.exit(8)
...
def makeFoldingEnergyDifferences(target, template, alignment, names=None, options=None):
    ...
    positions = [
        {target.name: {}, template.name: {}} for position in range(number_of_positions)
    ]
    ...
    for protein in [target, template]:
        ...
        for chain in protein.chains:
            for residue in chain.residues:
                while alignment[template.name][protein.name]["sequence"][
                    i_position
                ] in ["-", "?"]:
                ...
                position[protein.name]["contribution"] = residue.calculateFoldingEnergy(
                    options=options
                )
    ...
    for position in positions:
        str = "%5d  " % (i_position)
        for name in names:
            position[name]["difference"] = (
                position[name]["contribution"] - position[names[0]]["contribution"]
            )
...
def suggestMutation(
    positions=None, label=None, names=None, template=None, options=None
):
    """
...
def suggestMutations(positions, names=None, template=None, options=None):
    ...
    for position in sorted_positions:
        ...
        if position[names[1]]["difference"] > -0.5:
            ...
            determinant_labels = []
            mutation = makeMutationAddendum(
                target=target_label, template=template_label
            )
            # 1. get residue
            residue = template.getResidue(label=template_label)
            # 2. get determinants
            ...
            for determinant_label in determinant_labels:
                ...
                if target_label != "   gap   " and target_label[:3] not in [
                    ...
                    weight += residue.buried
            weight = int(100.0 * weight / (len(determinant_labels) + 1))
