    ...
    if axis.y != 0:
        if axis.x != 0:
            gamma = (
                -axis.x
                / abs(axis.x)
                * math.asin(axis.y / (math.sqrt(axis.x * axis.x + axis.y * axis.y)))
            )
        else:
            gamma = math.pi / 2.0
...
class multi_vector:
    ...
    def generic_operation(self, operation, other):
        ...
        self.res = multi_vector()
        for i in range(len(self.vectors)):
            self.res.vectors.append(
                eval("self.vectors[%d] %s other.vectors[%d]" % (i, operation, i))
            )
            self.res.keys.append(self.keys[i])
        return
...
def rotate_multi_vector_around_an_axis(theta, axis, v):
    ...
    for i in range(len(v.keys)):
        res.vectors.append(
            rotate_vector_around_an_axis(theta, axis.vectors[i], v.vectors[i])
        )
