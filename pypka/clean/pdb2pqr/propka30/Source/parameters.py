...
def getDesolvationDefault():
    ...
    """
    DesolvationDefault = {}
    DesolvationDefault["propka2"] = {
        "allowance": 400.00,
        "prefactor": -0.01,
        "fudge": 0.00,
        "scaled": False,
    }
    DesolvationDefault["ContactModel"] = {
        "allowance": 400.00,
        "prefactor": -0.01,
        "fudge": 0.00,
        "scaled": False,
    }
    DesolvationDefault["VolumeModel"] = {
        "allowance": 0.00,
        "prefactor": -13.50,
        "fudge": 0.00,
        "scaled": True,
    }
    DesolvationDefault["ScaledVolumeModel"] = {
        "allowance": 0.00,
        "prefactor": -20.00,
        "scaled": True,
    }
...
def getVanDerWaalsVolumes():
    ...
    """
    VanDerWaalsVolume = {
        "C": 1.40,  # 20.58 all 'C' and 'CA' atoms
...
def checkCooArgException(
    residue_coo, residue_arg, dpka_max=None, cutoff=None, version=None
):
    """
    ...
    for iter in ["shortest", "runner-up"]:
        ...
        for atom_coo in residue_coo.makeDeterminantAtomList(
            residue_arg.resName, version=version
        ):
            for atom_arg in residue_arg.makeDeterminantAtomList(
                residue_coo.resName, version=version
            ):
                fucked = False
        ...
        distance, f_angle, nada = lib.calculateAngleFactor(
            closest_coo_atom, closest_arg_atom, atom3
        )
...
class Version(object):
    ...
    def setDesolvation(
        self, label, prefactor=None, allowance=None, fudge=None, scaled=None
    ):
        """
    ...
    def calculateDesolvation(self, residue, atoms, verbose=True):
        ...
        if self.DesolvationModel == "propka2":
            Nmass, Emass, Nlocl, Elocl = calculate.originalDesolvation(
                residue, atoms, self, verbose=verbose
            )
        elif self.DesolvationModel == "ContactModel":
            Nmass, Emass, Nlocl, Elocl = calculate.contactDesolvation(
                residue, atoms, self, verbose=verbose
            )
        elif self.DesolvationModel in ["VolumeModel", "ScaledVolumeModel"]:
            Nmass, Emass, Nlocl, Elocl = calculate.radialVolumeDesolvation(
                residue, atoms, self, verbose=verbose
            )
        else:
    ...
    def checkExceptions(self, residue1, residue2):
        ...
        if resType1 == "COO" and resType2 == "ARG":
            exception, value = checkCooArgException_old(
                residue1, residue2, version=self
            )
            value = self.valueCooArgException
        elif resType1 == "ARG" and resType2 == "COO":
            exception, value = checkCooArgException_old(
                residue2, residue1, version=self
            )
            value = self.valueCooArgException
    ...
    def checkCoulombPair(self, residue1, residue2, distance):
        ...
        if (
            residue1.resType in self.coulomb_list
            and residue2.resType in self.coulomb_list
        ):
            # distance criteria
    ...
    def calculateSideChainEnergy(self, distance, dpka_max, cutoff, weight, f_angle):
        ...
        if True:
            ...
            prefactor = 1.00
        return calculate.SideChainEnergy(
            distance, prefactor * dpka_max, cutoff, f_angle
        )
...
class Jan01(Version):
    ...
    def __init__(self, verbose=True):
        ...
        self.setCoulomb(
            "Linear", cutoff=[4.0, 7.0], diel=None, scaled_diel=None, scaled=False
        )
...
class Oct13(Version):
    ...
    def __init__(self, verbose=True):
        ...
        self.setDesolvation(
            "VolumeModel", prefactor=-13.12, fudge=0.40, allowance=0.0, scaled=True
        )
        self.setCoulomb(
            "Coulomb", cutoff=[4.0, 10.0], diel=80.0, scaled_diel=True, scaled=False
        )
    ...
    def checkCoulombPair(self, residue1, residue2, distance):
        ...
        # check all Coulomb criteria!
        if (
            residue1.resType in self.pl_coulomb_list
            and residue2.resType in self.pl_coulomb_list
        ):
            # distance criteria
            if distance > self.coulomb_cutoff[1]:
                do_coulomb = False
...
class Jan15(Version):
    ...
    def __init__(self, verbose=True):
        ...
        self.desolv_cutoff = 15.50
        self.setDesolvation(
            "ContactModel", prefactor=-0.01, allowance=400.0, scaled=False
        )
        self.setCoulomb(
            "Linear", cutoff=[4.0, 7.0], diel=None, scaled_diel=None, scaled=False
        )
...
class Dec18(Version):
    ...
    def __init__(self, verbose=True):
        ...
        self.setCoulomb(
            "Linear", cutoff=[4.0, 7.0], diel=None, scaled_diel=None, scaled=True
        )
        self.doingBackBoneReorganization = False
...
class Aug24(Version):
    ...
    def __init__(self, verbose=True):
        ...
        self.desolv_cutoff = 20.00
        self.setDesolvation(
            "VolumeModel", prefactor=-6.75, fudge=0.00, allowance=0.0, scaled=False
        )
        self.setCoulomb(
            "Linear", cutoff=[4.0, 7.0], diel=80.0, scaled_diel=None, scaled=True
        )
        self.doingBackBoneReorganization = True
...
class Aug30(Version):
    ...
    def __init__(self, verbose=True):
        ...
        self.desolv_cutoff = 20.00
        self.setDesolvation(
            "VolumeModel", prefactor=-14.75, fudge=0.00, allowance=0.0, scaled=True
        )
        self.setCoulomb(
            "Linear", cutoff=[4.0, 7.0], diel=80.0, scaled_diel=None, scaled=True
        )
...
class Aug31(Version):
    ...
    def __init__(self, verbose=True):
        ...
        self.desolv_cutoff = 20.00
        self.setDesolvation(
            "VolumeModel", prefactor=-13.50, fudge=0.25, allowance=0.0, scaled=True
        )
        self.setCoulomb(
            "Linear", cutoff=[4.0, 7.0], diel=80.0, scaled_diel=None, scaled=True
        )
...
class Sep08(Version):
    ...
    def __init__(self, verbose=True):
        ...
        self.desolv_cutoff = 20.00
        self.setDesolvation(
            "VolumeModel", prefactor=-13.75, fudge=0.20, allowance=0.0, scaled=True
        )
        self.setCoulomb(
            "Coulomb", cutoff=[4.0, 10.0], diel=20.0, scaled_diel=False, scaled=True
        )
        self.doingBackBoneReorganization = True
...
class Oct14(Version):
    ...
    def __init__(self, verbose=True):
        ...
        self.desolv_cutoff = 20.00
        self.setDesolvation(
            "VolumeModel", prefactor=-13.25, fudge=0.325, allowance=0.0, scaled=True
        )
        self.setCoulomb(
            "Coulomb", cutoff=[4.0, 10.0], diel=80.0, scaled_diel=True, scaled=False
        )
    ...
    def checkExceptions(self, residue1, residue2):
        ...
        if resType1 == "COO" and resType2 == "ARG":
            ...
            exception, value = checkCooArgException(
                residue1, residue2, dpka_max=dpka_max, cutoff=cutoff, version=self
            )
        elif resType1 == "ARG" and resType2 == "COO":
            dpka_max, cutoff = self.getSideChainParameters(residue1, residue2)
            exception, value = checkCooArgException(
                residue2, residue1, dpka_max=dpka_max, cutoff=cutoff, version=self
            )
        elif resType1 == "COO" and resType2 == "COO":
            exception, value = checkCooCooException(residue1, residue2)
            value = self.valueCooCooException
...
class Jan01_old:
    ...
    def checkExceptions(self, residue1, residue2):
        ...
        if name1 == "COO" and name2 == "ARG":
            ...
            exception, value = checkCysCysException(residue1, residue2)
        elif (name1 == "COO" and name2 == "HIS") or (name1 == "HIS" and name2 == "COO"):
            exception, value = checkCooHisException(residue1, residue2)
        elif (name1 == "CYS" and name2 == "HIS") or (name1 == "HIS" and name2 == "CYS"):
            exception, value = checkCysHisException(residue1, residue2)
        else:
            """ do nothing, no exception for this pair """
