...
def optimizeMutationDeterminants(
    protein, mutation=None, atoms=None, alignment=None, version=None, options=None
):
    """
    ...
    if options.mutator.label in ["alignment", "overlap"] and alignment == None:
        ...
        alignment = readAlignmentFiles(
            filenames=options.alignment, mesophile=protein.name, options=options
        )
    ...
    if isinstance(mutation, str):
        ...
        combinatorial_mutations = generateCombinatorialDictionaryMutations(
            dict_mutation
        )
    else:
    ...
    for combinatorial_mutation in combinatorial_mutations:
        newProtein = makeMutatedProtein(
            protein,
            mutation=combinatorial_mutation,
            atoms=atoms,
            alignment=alignment,
            options=options,
        )
...
def optimizeMultipleMutations(
    protein, mutations=None, atoms=None, alignment=None, version=None, options=None
):
    """
    ...
    for combinatorial_mutation in combinatorial_mutations:
        newProtein = makeMutatedProtein(
            protein, mutation=combinatorial_mutation, atoms=atoms, options=options
        )
        newProtein.calculatePKA(version=version, options=options)
        dG_mut = newProtein.calculateFoldingEnergy(options=options)
...
def postProcessCombinatorialMutation(proteins, options=None):
    ...
    pka_print(
        "%8s %6s\n%8.2lf %6.2lf  %s"
        % ("dG_fold", "ddG_fold", dG_ref, dG_ref - dG_ref, "WT")
    )
...
def makeMutatedProtein(
    protein, mutation=None, atoms=None, alignment=None, options=None
):
    """
    ...
    if isinstance(mutation, str):
        ...
        if pdbcode == None:
            ...
            mutation = remakeStringMutation(
                mutation, protein.atoms.keys(), atoms[pdbcode].keys()
            )
        dict_mutation = recastIntoDictionary(
            mutation, alignment=alignment, protein=protein
        )
    ...
    if options.mutator.label == "alignment":
        newAtoms = mutateAtomsDictionaryUsingAlignment(
            protein, mutation=dict_mutation, atoms=atoms, options=options
        )
        ...
        newAtoms = mutateAtomsDictionaryUsingOverlap(
            protein, mutation=dict_mutation, atoms=atoms, options=options
        )
        printMutation(dict_mutation)
        name = "%s_mut" % (protein.name)
        ...
        seqfile = "x-ray.seq"
        sequence = protein.makeSequence(
            mutation=dict_mutation, program="scwrl", options=options
        )
        ...
        output.writeJackalScapFile(
            mutationData=mutation_data, filename=scapfile, options=options
        )
...
def executeJackal(pdbfile, scapfile, newfile=None, options=None):
    ...
    cmd = "scap -prm %d -min %d -ini %d -rtm %d  %s %s" % (
        prm,
        min,
        ini,
        rtm,
...
def mutateAtomsDictionaryUsingAlignment(
    protein, atoms=None, mutation=None, options=None
):
    """
...
def singleMutateAtomsDictionary(
    atoms=None, template=None, mutation=None, type=None, configs=None, options=None
):
    """
    ...
    backbone = setupBackBoneDictionary(
        target=atoms[target_chainID][target_label],
        template=template[code][template_chainID][template_label],
    )
    ...
    for name in backbone_names:
        ...
        if type == "side-chain":
            ...
            new_atom = template_atom.makeCopy(
                resNumb=resNumb, chainID=target_chainID, configs=configs
            )
        ...
        for key in protein.atoms[chainID]["keys"]:
            ...
            if key in mutation.keys():
                ...
                if options.mutator.type == "back-track":
                    dX, dY, dZ = getBackTrackTranslation(
                        protein.atoms[chainID][key], atoms[code][chainID][key1]
                    )
                resNumb = int(key[3:7])
                resName = key2[:3]
                ...
                for atom in atoms[code][template_chainID][key1]:
                    if atom.name in ["N", "CA", "C", "O"]:
                        ...
                        if options.mutator.type == "all":
                            ...
                            y = atom.y + dY
                            z = atom.z + dZ
                            newAtom = atom.makeCopy(
                                resNumb=resNumb, x=x, y=y, z=z, chainID=chainID
                            )
                            atomList.append(newAtom)
                    else:
                        # copy the thermophile side-chain atom
                        newAtom = atom.makeCopy(resNumb=resNumb, chainID=chainID)
                        newAtom.checkConfigurations(
                            configurations=protein.configurations
                        )
                        atomList.append(newAtom)
...
def splitIntoMutationData(generic_mutation):
    ...
    if isinstance(generic_mutation, str):
        ...
        for key in generic_mutation.keys():
            ...
            code1, resName = lib.convertResidueCode(
                resName=generic_mutation[key]["target"][:3]
            )
            resNumb = int(generic_mutation[key]["target"][3:7])
            code2, resName = lib.convertResidueCode(
                resName=generic_mutation[key]["label"][:3]
            )
    ...
    if len(mutation_data) > 0:
        ...
        for mutation in mutation_list:
            ...
            for single_mutation in splitIntoSingleMutations(mutation_string):
                chainID1, chainID2, mutation_label = splitSingleMutation(
                    single_mutation
                )
...
def recastIntoDictionary(
    input_mutation=None, alignment=None, protein=None, chains=None
):
    """
        ...
        for single_mutation in splitIntoSingleMutations(mutation_string):
            ...
            if alignment == None or mesophile == None:
                ...
                for i in range(len(alignment[thermophile][mesophile]["sequence"])):
                    if (
                        alignment[thermophile][mesophile]["sequence"][i]
                        in "ARNDCQEGHILKMFPSTWYV"
                    ):
                        resNumb_mesophile += 1
                    if (
                        alignment[thermophile][thermophile]["sequence"][i]
                        in "ARNDCQEGHILKMFPSTWYV"
                    ):
...
def splitSingleMutation(single_mutation):
    ...
    if separator == None:
        ...
        pka_print(
            'could not split mutation "%s" correctly in splitSingleMutation()'
            % (single_mutation)
        )
        sys.exit(8)
...
def remakeStringMutation(generic_mutation, keys1=None, keys2=None):
    ...
    if isinstance(generic_mutation, str):
        ...
        pka_print(
            "don't know how to deal with dictionary-mutations in checkStringMutation()"
        )
        sys.exit(8)
    ...
    if isinstance(mutation, str):
        ...
        for key in mutation.keys():
            ...
            if "label" not in mutation[key] and "template" in mutation[key]:
                ...
                mutation[key]["label"] = "%s%s" % (
                    mutation[key]["template"][:3],
                    label[3:],
                )
...
def makePositionDictionaryFromResidues(original_residue=None, template_residue=None):
    ...
    labels = []
    translate = {}
    position = {"target": {}, "new": {}, "old": {}, "template": {}}
    ...
    for atom in template_residue.atoms:
        if atom.name[0] != "H":
            ...
            if ref_to_atom == None:
                ...
                position["target"][atom.name] = [
                    ref_to_atom.x + translate["target"][x],
                    ref_to_atom.y + translate["target"][y],
                    ref_to_atom.z + translate["target"][z],
                ]
                position["template"][atom.name] = [
                    atom.x + translate["template"][x],
...
def makePositionDictionaryFromAtoms(
    target=None, template=None, corresponding_names=None
):
    """
    creating a position dictionary for test-rotations and rmsd calculations
    """
    labels = []
    translate = {"target": [0.00, 0.00, 0.00], "template": [0.00, 0.00, 0.00]}
    position = {"target": {}, "new": {}, "old": {}, "template": {}}
...
def mutateAtomsDictionaryUsingOverlap(
    protein=None, atoms=None, mutation=None, options=None
):
    """
    ...
    for key in mutation.keys():
        ...
        center, translate, position = makePositionDictionaryFromAtoms(
            target=protein.atoms[target_chainID][target_label],
            template=atoms[code][template_chainID][template_label],
            corresponding_names=corresponding_names[template_label[:3]][
                target_label[:3]
            ],
        )
        ...
        # -- rotation 1 ---
        cross_product = makeCrossProduct(
            position["target"]["N"], position["template"]["N"]
        )
        ...
        rotateAtoms(
            newAtoms[target_chainID][label], cross_product, -theta1, center=["CA"]
        )

        #  ---  rotation 2  ---
        target_cross_product = makeCrossProduct(
            position["target"]["N"], position["target"]["C"]
        )
        template_cross_product = makeCrossProduct(
            position["template"]["N"], position["template"]["C"]
        )
        ...
        if (
            makeScalarProduct(
                position["template"]["C"],
                makeCrossProduct(position["target"]["C"], position["target"]["N"]),
            )
            < 0.00
        ):
        ...
        rotatePosition(
            position["template"], position["target"]["N"], -theta2, center=["CA"]
        )
        rotateAtoms(
            newAtoms[target_chainID][label],
            position["target"]["N"],
            -theta2,
            center=["CA"],
        )
        ...
        for name in position["template"].keys():
            center.append(name)
            position["new"][name] = [
                position["template"][name][0],
                position["template"][name][1],
                position["template"][name][2],
            ]
            position["old"][name] = [
                position["template"][name][0],
                position["template"][name][1],
            ...
            if rmsd_new < rmsd_old:
                ...
                for name in position["old"].keys():
                    ...
                    if False:
                        ...
                        chainNumb = ord("B")
                        pka_print(
                            "ATOM    669  %-3s GLU %s  89    %8.3lf%8.3lf%8.3lf"
                            % ("H", chr(chainNumb), Rx, Ry, Rz)
                        )
...
def makeMutationLight(
    original_protein=None, template_protein=None, mutation=None, iterations=500
):
    """
    This routine overlaps two residues based on an array of atom labels, 'center'
    ...
    for label in mutation.keys():
        ...
        template_residue = template_protein.getResidue(
            label=mutation[label]["template"]
        )
        copied_residue = template_residue.makeCopy(
            chainID=original_residue.chainID, resNumb=original_residue.resNumb
        )

        # set up position-dictionary for corresponding atoms used for evaluating rmsd
        center, translate, position = makePositionDictionary(
            original_residue, template_residue
        )
        ...
        if False:
            for key in position["target"].keys():
                ...
                pka_print(
                    "ATOM    669  %-3s %s A  89    %8.3lf%8.3lf%8.3lf"
                    % (key, original_residue.resName, Rx, Ry, Rz)
                )
            for key in position["template"].keys():
                Rx, Ry, Rz = position["template"][key]
                pka_print(
                    "ATOM    669  %-3s GLU B  89    %8.3lf%8.3lf%8.3lf"
                    % ("H", Rx, Ry, Rz)
                )
        ...
        if False:
            for key in position["template"].keys():
                Rx, Ry, Rz = position["template"][key]
                pka_print(
                    "ATOM    669  %-3s GLU C  89    %8.3lf%8.3lf%8.3lf"
                    % ("H", Rx, Ry, Rz)
                )
        ...
        for key in position["template"].keys():
            position["new"][key] = [
                position["template"][key][0],
                position["template"][key][1],
                position["template"][key][2],
            ]
            position["old"][key] = [
                position["template"][key][0],
                position["template"][key][1],
