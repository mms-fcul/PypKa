...
class Residue:
    ...
    def __init__(
        ...
        self.type = (
            None  # 'amino-acid' / 'ion' / 'ligand' / 'N-terminus' / 'C-terminus'
        )
        self.buried = None
        self.location = None
        self.determinants = [[], [], []]
        ...
        self.center_atoms = (
            []
        )  # list of atoms constituting the 'residue center', needed!
        self.default_key = None  # the 'default' configuration if not all are there
        self.configurations = (
            []
        )  # keys to the atom configurations belonging to this residue
    ...
    def checkOXT(self):
        ...
        if OXT == None:
            ...
            if O == None or CA == None or C == None:
                pka_print(
                    "ERROR: cannot create OXT atom - missing CA, C, or O atoms; please correct pdbfile"
                )
                sys.exit(8)
            dX = -((CA.x - C.x) + (O.x - C.x))
            dY = -((CA.y - C.y) + (O.y - C.y))
        ...
        for atom in self.atoms:
            for configuration in self.configurations:
                if key not in atom.configurations:
                    atom.configurations[configuration] = atom.configurations[
                        self.default_key
                    ]
    ...
    def makeDeterminantAtomList(self, resType=None, type=None, version=None):
        ...
        if self.resName not in version.atomInteractionList[pair_type]:
            pka_print(
                "cannot find atomInteractionList for residue %s in residue.makeDeterminantAtomList()"
                % (self.resName)
            )
            sys.exit(9)
    ...
    def calculateTotalPKA(self):
        ...
        self.pKa_pro = (
            self.pKa_mod + self.Emass + self.Elocl + back_bone + side_chain + coulomb
        )
    ...
    def calculateIntrinsicPKA(self):
        ...
        self.intrinsic_pKa = (
            self.pKa_mod + self.Emass + self.Elocl + back_bone + side_chain
        )
    ...
    def calculateTitrationCurve(self, grid=[0.0, 14.0, 0.10]):
        """
    ...
    def getDeterminantString(self):
        ...
        number_of_determinants = (
            number_of_sidechain + number_of_backbone + number_of_coulomb
        )
        number_of_lines = max(
            1, number_of_sidechain, number_of_backbone, number_of_coulomb
        )
        outsting = ""
        ...
        for atom in self.atoms:
            ...
            for key in atom.configurations.keys():
                for xyz in range(3):
                    ...
                    for i in range(3):
                        new_position[xyz] += (
                            rotation_matrix[xyz][i] * atom.configurations[key][i]
                        )
                for xyz in range(3):
