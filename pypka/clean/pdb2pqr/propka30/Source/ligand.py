        ...
        for atom in self.atoms:
            ...
            if atom.get_element() == "C":
                ...
                if (
                    ...
                    if (
                        len(atom.bonded_atoms[i1].bonded_atoms) == 1
                        and len(atom.bonded_atoms[i2].bonded_atoms) == 1
                    ):
                        # self.set_type(atom.bonded_atoms[i1], 'O.co2+')
                        self.set_type(
                            atom.bonded_atoms[i1], "O.co2"
                        )  # SH: problems in 1a3w
                ...
                if len(atom.bonded_atoms) <= 2:
                    for b in atom.bonded_atoms:
                        if (
                            self.my_bond_maker.squared_distance(atom, b)
            ...
            if atom.get_element() == "O":
                ...
                if len(atom.bonded_atoms) == 1:
                    if (
                        self.my_bond_maker.squared_distance(atom, atom.bonded_atoms[0])
    ...
    def identify_ring(self, this_atom, original_atom, number, past_atoms):
        ...
        for atom in this_atom.bonded_atoms:
            ...
            if atom not in past_atoms:
                these_return_atoms = self.identify_ring(
                    atom, original_atom, number, past_atoms
                )
                if len(these_return_atoms) > 0:
                    if (
                        len(return_atoms) > len(these_return_atoms)
