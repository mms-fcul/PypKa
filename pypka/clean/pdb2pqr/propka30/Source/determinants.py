...
def setDeterminants(propka_residues, version=None, options=None):
    ...
    for residue1 in propka_residues:
        for residue2 in propka_residues:
            ...
            if (
                distance < version.sidechain_cutoff
                or distance < version.coulomb_cutoff[1]
            ):
                try:
                    do_pair, iterative_interaction = version.interaction[
                        residue1.resType
                    ][residue2.resType]
                ...
                if do_pair == True:
                    if iterative_interaction == True:
                        iterative.addtoDeterminantList(
                            residue1,
                            residue2,
                            distance,
                            iterative_interactions,
...
def addSidechainDeterminants(residue1, residue2, version=None):
    ...
    if distance < cutoff[1]:
        ...
        if exception == True:
            ...
            value = version.calculateSideChainEnergy(
                distance, dpka_max, cutoff, weight, f_angle
            )
        ...
        if residue.location != "BONDED":
            ...
            for interaction in interactions:
                ...
                if distance < cutoff[1] and f_angle > 0.001:
                    ...
                    value = residue.Q * calculate.HydrogenBondEnergy(
