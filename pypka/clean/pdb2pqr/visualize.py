...
def initVars():
    ...
    if not form.has_key("jobid"):
        ...
        if APBS_OPAL_URL != "":
            ...
            apbsOpalJobIDFile = open(
                "%s%s%s/apbs_opal_job_id" % (INSTALLDIR, TMPDIR, jobid)
            )
            ...
            for file in resp._outputFile:
                ...
                if fileName != "Standard Output" and fileName != "Standard Error":
                    urllib.urlretrieve(
                        file._url, "%s%s%s/%s" % (INSTALLDIR, TMPDIR, jobid, fileName)
                    )
...
<html>
    ...
    redirectURL=WEBSITE, website=WEBSITE
)
...
<!DOCTYPE html>
    ...
    <body>
        ...
        trackingevents=getEventTrackingString(
            category="apbs", action="visualize", label=str(os.environ["REMOTE_ADDR"])
        ),
