...
class ResInter(object):
    ...
    def create_all_protonated(self):
        residueSet = get_residue_titration_set_protonated(
            self.routines.protein.getResidues()
        )
        self.process_residue_set(
            residueSet,
    ...
    def create_all_single_unprotonated(self):
        combinations = residue_set_single_unprotonated_combinations(
            self.routines.protein.getResidues()
        )
    ...
    def create_all_pair_unprotonated(self):
        combinations = residue_set_pair_unprotonated_combinations(
            self.routines.protein.getResidues()
        )
    ...
    def count_combinations(self):
        ...
        allProtonated = get_residue_titration_set_protonated(
            self.routines.protein.getResidues()
        )
    ...
    def write_resinter_output(self):
        ...
        for resultKey in sorted(self.pairEnergyResults.iterkeys()):
            self.output.write(
                resultKey + " " + str(self.pairEnergyResults[resultKey]) + "\n"
            )

        self.routines.write(
            str(self.combinationCount) + " residue combinations tried\n"
        )

    def process_residue_set(
        self,
        residueSet,
        clean=False,
        neutraln=False,
        ...
        txt = "Running combination {0} of {1}\n".format(
            self.combinationCount, self.totalCombinations
        )
        ...
        for newResidueName, oldResidue, index in izip(
            residueSet, self.routines.protein.getResidues(), count()
        ):
            ...
            newResidue = self.routines.protein.createResidue(
                residueAtoms, newResidueName
            )
