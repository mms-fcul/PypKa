...
def compareParsedAtoms(atom1, atom2):
    return atom1[0:1] == atom2[0:1] and all(
        abs(x - y) < 0.1 for x, y in izip(atom1[2], atom2[2])
    )
...
def ComparePQRAction(outputFileName, testFileName, correctFileName, has_chain=False):
    ...
    with open(testFileName) as testFile:
        with open(correctFileName) as correctFile:
            ...
            correct_total = 0.0
            test_total = 0.0
            for testAtom, correctAtom in izip_longest(
                testAtoms, correctAtoms, fillvalue=None
            ):
            ...
            if check_error and abs(test_total - correct_total) > 20.0:
                results.append(
                    "TEST ERROR: Result file does not match target close enough!\n"
                )
                failure = True
...
def CompareTitCurvesAction(outputFileName, testDirName, correctDirName):
    ...
    for name in correct_data:
        ...
        if test_curve is None:
            results.append(
                "ERROR: test results missing curve for residue " + name + "\n"
            )
            continue
        ...
        if bad_point_count > 0:
            results.append(
                "ERROR: test curve {name} has {count}"
                " bad points with {total} cumulative error.\n".format(
                    name=name, count=bad_point_count, total=total_error
                )
            )
...
def ComparePROPKAAction(outputFileName, testFileName, correctFileName):
    ...
    with open(testFileName) as testFile:
        with open(correctFileName) as correctFile:
            ...
            correct_total = 0.0
            test_total = 0.0
            for testPKA, correctPKA in izip_longest(
                testPKAs, correctPKAs, fillvalue=None
            ):
...
def CompareDirectoryFunc(outputFileName, targetdir, sourcedir):
    return 'Comparing directories ("%s", "%s") -> %s' % (
        targetdir,
        sourcedir,
        outputFileName,
    )
