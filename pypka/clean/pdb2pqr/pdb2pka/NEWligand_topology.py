    ...
    def __init__(self, lines, MOL2FLAG):
        ...
        if MOL2FLAG == False:
            ...
            for atom1 in self.atoms.keys():
                ...
                for atom2 in self.atoms.keys():
                    ...
                    self.dists[atom1][atom2] = length(
                        self.atoms[atom1]["coords"] - self.atoms[atom2]["coords"]
                    )
            ...
            for line in lines:
                ...
                self.atoms[name] = {
                    "coords": numpy.array([float(line.x), float(line.y), float(line.z)])
                }
                ...
                self.atoms[name]["nbhvy"] = len(
                    [x for x in self.atoms[name]["lBondedAtoms"] if x.sybylType != "H"]
                )
                # number of bonds (including hydrogens)
                self.atoms[name]["nbds"] = len(self.atoms[name]["lBonds"])
                # number of bonded hydrogens
                self.atoms[name]["nbhyd"] = (
                    self.atoms[name]["nbds"] - self.atoms[name]["nbhvy"]
                )
                # element
    ...
    def find_titratable_groups(self):
        ...
        def match(t, l):
            ...
            for ligand_atoms in l:
                for template_atoms in t:
                    if (
                        templates[current_template][template_atoms]["sybylType"]
                        == self.atoms[ligand_atoms]["sybylType"]
                    ):
            ...
            if len(nodes) != 0:
                ...
                for i in nodes:
                    for j in nodes:
                        ...
                        if (
                            ...
                            and (
                                templates[current_template][i.at_idx2]["alreadyvisited"]
                                != True
                            )
                        ):
                            AtomNameList.append(i.at_idx1)
                            ...
                            templates[current_template][i.at_idx2][
                                "alreadyvisited"
                            ] = True
                            AtomNameListList.append(AtomNameList)
                ...
                if str(AGL[0, 0]) != "0":
                    AGL = numpy.zeros(
                        (len(AtomNameListList), len(AtomNameListList)), numpy.int
                    )
                ...
                for daseine in AtomNameListList:
                    ...
                    for dasandere in AtomNameListList:
                        if (daseine[0] == dasandere[0]) and (
                            daseine[1] == dasandere[1]
                        ):
                            AGL[daseinecounter][dasanderecounter] = 1
                            # bonded
                        elif (
                            dasandere[0] in self.atoms[daseine[0]]["bonds"]
                            and dasandere[1]
                            in templates[current_template][daseine[1]]["neighbours"]
                        ):
                            AGL[daseinecounter][dasanderecounter] = 1
                        # non-bonded atoms in template must also be non-bonded in the ligand
                        elif (
                            dasandere[0] not in self.atoms[daseine[0]]["bonds"]
                            and dasandere[1]
                            not in templates[current_template][daseine[1]]["neighbours"]
                        ):
                            AGL[daseinecounter][dasanderecounter] = 1
                        dasanderecounter += 1
                    daseinecounter += 1
        ...
        for current_template in templates.keys():
            ...
            for ee in output[0]:
                ...
                matchedLigAtoms = []
                temptemp = []
                if len(temp_templateList) == len(ligList) and (
                    len(ee) == len(templates[current_template].keys())
                ):
                    for xxee in ee:
                    ...
                    dict_of_matched_lig_fragments[dictCounter][
                        "type"
                    ] = templates_attributes[output[2]]["type"]
                    dict_of_matched_lig_fragments[dictCounter][
                        "modelpka"
                    ] = templates_attributes[output[2]]["modelpka"]
                    ...
                    dict = {}
                    for looping_template in templates_attributes[output[2]][
                        "titratableatoms"
                    ]:
                        for looping_lig in output[1]:
                            # print looping_lig
                            if looping_lig[1] == looping_template and (
                                looping_lig in temptemp
                            ):
                ...
                if counter < len(AllCliqueList):
                    ...
                    for comparing in range(internalCounter, len(AllCliqueList)):
                        ...
                        if xxxx != AllCliqueList[internalCounter]:
                            if (
                                len(
                                    set(xxxx).intersection(
                                        set(AllCliqueList[internalCounter])
                                    )
                                )
                                > 0
                            ):
                                if (
                                    len(xxxx) >= len(AllCliqueList[internalCounter])
                                    and xxxx not in NonRedundantCliques
                                ):  # >= INSTEAD of >
                                    # piperidine case!
                                    ...
                                    if len(NonRedundantCliques) != 0:
                                        ...
                                        for (
                                            possiblyredundantentries
                                        ) in NonRedundantCliques:
                                            if set(possiblyredundantentries).issubset(
                                                set(xxxx)
                                            ):
                                                print NonRedundantCliques
                                                NonRedundantCliques.remove(
                                                    possiblyredundantentries
                                                )
                                                NonRedundantCliques.append(xxxx)
                                                print NonRedundantCliques
                                            elif set(xxxx).issubset(
                                                set(possiblyredundantentries)
                                            ):
                                ...
                                # the other way around
                                    ...
                                    len(AllCliqueList[internalCounter]) >= len(xxxx)
                                    and AllCliqueList[internalCounter]
                                    not in NonRedundantCliques
                                ):
                                    if len(NonRedundantCliques) != 0:
                                        for (
                                            possiblyredundantentries
                                            ...
                                            if set(possiblyredundantentries).issubset(
                                                ...
                                                NonRedundantCliques.append(
                                                    AllCliqueList[internalCounter]
                                                )
                                            elif set(
                                                AllCliqueList[internalCounter]
                                            ).issubset(set(possiblyredundantentries)):
                                                # print "found subset which is not added to the list"
                                                pass
                                    else:
                                        NonRedundantCliques.append(
        ...
        for allCl in dict_of_matched_lig_fragments:
            if (
                dict_of_matched_lig_fragments[allCl]["matchedligatoms"]
                == NonRedundantCliques[0]
            ):
                print "WE MATCHED", dict_of_matched_lig_fragments[allCl]["templatename"]
                print "matchedligatoms            : ", dict_of_matched_lig_fragments[
                    allCl
                ]["matchedligatoms"]
                print "type                       : ", dict_of_matched_lig_fragments[
                    allCl
                ]["type"]
                print "modelpka                   : ", dict_of_matched_lig_fragments[
                    allCl
                ]["modelpka"]
                print "titratableatoms            : ", dict_of_matched_lig_fragments[
                    allCl
                ]["titratableatoms"]
                print "matching atoms             : ", dict_of_matched_lig_fragments[
                    allCl
                ]["matching_atoms"]
