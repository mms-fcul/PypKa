...
class get_ligand_topology:
    ...
    def __init__(self, lines, MOL2FLAG):
        ...
        if MOL2FLAG == False:
            ...
            for line in lines:
                ...
                self.atoms[name] = {
                    "coords": Numeric.array(
                        [float(line.x), float(line.y), float(line.z)]
                    )
                }
    ...
    def find_titratable_groups(self):
        ...
        print "# overall rings (including potentially fused rings) :", len(
            sorted_ring_list
        )
        stop  ## PC 03.01.06
        #
    ...
    def matched_atom_types(atom2match, t):
        ...
        def match(t, l, already_visited=[], type_matches=[]):
            for counter in range(len(atoms)):
                ...
                matched_atom_in_template, nbs_in_template = matched_atom_types(
                    at_lig, t
                )
                if matched_atom_in_template != None:
                    for entries in matched_atom_in_template:
                        ...
                        for sybyl_bonded_at in self.atoms[at_lig]["lBondedAtoms"]:
                            ...
                            if len(ligand_set.difference(template_set)) == 0 and len(
                                ligand_list
                            ) == len(nbs_in_template):
                                for entry in matched_atom_in_template:
                                    print "%3d" % (
                                        ...
                                        at_lig,
                                        self.atoms[at_lig]["sybylType"],
                                        ligand_list,
                                    ), "template %s %s %s %s" % (
                                        matched_atom_in_template,
                                        t[entry]["sybylType"],
                                        nbs_in_template,
                                        t[entry]["neighbours"],
                                    )
                                    for neighboured_template_atoms in t[entry][
                                        "neighbours"
                                    ]:
                                        print neighboured_template_atoms, t[
                                            neighboured_template_atoms
                                        ]["sybylType"], t[neighboured_template_atoms][
                                            "sybyl_neighbours"
                                        ]
                                    for neighboured_ligand_atoms in self.atoms[at_lig][
                                        "lBondedAtoms"
                                    ]:
        ...
        def matched_atom_types2(
            ...
            if (
                len(stored_nbs_of_atom2match) != 0
                and stored_nbs_of_atom2match[-1] == atom2match
            ):
            ...
            for at in t.keys():
                ...
                if (
                    t[at]["sybylType"] == self.atoms[atom2match]["sybylType"]
                    ...
                    if len(diff) == 0:
                        ...
                        for nbat in self.atoms[atom2match]["bonds"]:
                            ...
                            for id in range(len(self.atoms[atom2match]["bonds"]) - 1):
                                next_nbat_id = id + 1
                                next_nbat_at = self.atoms[atom2match]["bonds"][
                                    next_nbat_id
                                ]
                                if next_nbat_at not in already_visited:
                                    ...
                                    already_visited.append(
                                        self.atoms[atom2match]["bonds"][next_nbat_id]
                                    )
                                    matched_atom_types2(
                                        self.atoms[atom2match]["bonds"][next_nbat_id], t
                                    )
                                else:
                                    pass
        ...
        def gothroughallnbsofmatchlistatom(
            stored_nbs_of_atom2match, t, already_visited, hit_list
        ):
            ...
            for ent_lig in stored_nbs_of_atom2match:
                ...
                if ent_lig not in already_visited:
                    ...
                    for at in t.keys():
                        if (
                            t[at]["sybylType"] == self.atoms[ent_lig]["sybylType"]
                            and ent_lig not in matchlist
                        ):
                            matchlist.append(at)
                    for matches in matchlist:
                        if (
                            len(
                                Set(t[matches]["sybyl_neighbours"]).difference(
                                    Set(createsybyllistonthefly(ent_lig))
                                )
                            )
                            == 0
                        ):
                            ...
                            for putative_next_atom2match in self.atoms[ent_lig][
                                "bonds"
                            ]:
                                if (
                                    putative_next_atom2match
                                    not in putative_next_a2m_list
                                ):
                                    putative_next_a2m_list.append(
                                        putative_next_atom2match
                                    )
                        else:
        ...
        def matchatomtypeintemplateandgetliglist(
            atom2match,
            t,
            stored_nbs_of_atom2match=[],
            been_here_flag=False,
            already_visited=[],
            hit_list=[],
        ):
            ...
            if atom2match in stored_nbs_of_atom2match:
                (
                    ...
                    putative_next_a2m_list,
                    hit_list,
                ) = gothroughallnbsofmatchlistatom(
                    stored_nbs_of_atom2match, t, already_visited, hit_list
                )
            ...
            if been_here_flag == True:
                ...
                for next_at in putative_next_a2m_list:
                    ...
                    matchatomtypeintemplateandgetliglist(
                        next_at, t, been_here_flag=True
                    )
            matchlist = []
            for at in t.keys():
                if (
            ...
            for entries in matchlist:
                ...
                if (
                    len(
                        Set(t[entries]["sybyl_neighbours"]).difference(
                            Set(createsybyllistonthefly(atom2match))
                        )
                    )
                    == 0
                ):
                    ...
                    for nbs in stored_nbs_of_atom2match:
                        ...
                        #
                        matchatomtypeintemplateandgetliglist(
                            nbs, t, stored_nbs_of_atom2match
                        )
        ...
        for current_template in templates.keys():
            match2(
                templates[current_template], atoms, start_atom=atoms[4]
            )  # start_atom should be 0
