...
titratable_residue_data = {
    "ARG": {
        "model_pka": 13.0,
        "ionizable": 1,
        "tautomers": {"deprotonated": ("1+2+3+4",), "protonated": ("1+2+3+4+5",)},
    },
    "ASP": {
        "model_pka": 3.9,
        "ionizable": -1,
        "tautomers": {"protonated": ("1", "2", "3", "4"), "deprotonated": ("0",)},
    },
    "GLU": {
        "model_pka": 4.1,
        "ionizable": -1,
    ...
    "LYS": {
        "model_pka": 10.4,
        "ionizable": 1,
        "tautomers": {"protonated": ("1",), "deprotonated": ("0",)},
    },
    "TYR": {
        "model_pka": 9.6,
        "ionizable": 1,
    ...
    "NTR": {
        "model_pka": 8.3,
        "ionizable": 1,
        "tautomers": {"deprotonated": ("1", "2"), "protonated": ("1+2",)},
    },
    "HIS": {
        "model_pka": 6.6,
        "ionizable": 1,
        "tautomers": {"protonated": ("1", "2"), "deprotonated": ("1+2",)},
    },
}
all_tautomers = set()
...
class ResidueVariable(object):
    ...
    def get_prot_and_deprot_instances(self):
        ...
        prot = [
            ...
            if "PROTONATED" not in key and value.protonated
        ]
        deprot = [
            value
            for key, value in self.instances.items()
            if "PROTONATED" not in key and not value.protonated
        ]
        return prot, deprot
...
class ProteinComplex(object):
    ...
    def add_interaction_energy_pair(
        self, instance1, instance2, energy, normalized=False
    ):
        "Insert interaction energy pair."
        ie = (
            self.normalized_interaction_energies
            if normalized
            else self.interaction_energies
        )
    ...
    def consolidate(self):
        ...
        for v, w in combinations(iter(self.residue_variables.items()), 2):
            ...
            v_stuff = (
                (v_protinated, v_prot_consolidated),
                (v_unprotonated, v_deprot_consolidated),
            )
            w_stuff = (
                (w_protinated, w_prot_consolidated),
            ...
            for v_product, w_product in product(v_stuff, w_stuff):
                ...
                self.add_interaction_energy_pair(
                    v_consolidated, w_consolidated, min_energy
                )
        ...
        for v, w in permutations(iter(self.residue_variables.items()), 2):
            ...
            (
                other_protinated,
                other_unprotonated,
            ) = other_residue.get_prot_and_deprot_instances()
            ...
            (
                his_protinated,
                his_unprotonated,
            ) = his_residue.get_prot_and_deprot_instances()
            his_stuff = his_protinated + his_unprotonated
            other_stuff = (
                (other_protinated, other_prot_consolidated),
            ...
            for his_instance, other_product in product(his_stuff, other_stuff):
                ...
                self.add_interaction_energy_pair(
                    his_instance, other_consolidated, min_energy
                )
        ...
        for key, residue in self.residue_variables.items():
            ...
            residue.instances = OrderedDict(
                (k, v) for k, v in residue.instances.items() if "PROTONATED" in k
            )
    ...
    def divide_his(self):
        ...
        for key, residue in items:
            ...
            energy = (
                old_instance_12.energy - old_instance_1.energy - old_instance_2.energy
            )
        ...
        for v, w in product(iter(self.residue_variables.items()), repeat=2):
            ...
            if other_name == "HId":
                ...
                if his_name == "HIe":
                    energy = self.interaction_energies[
                        old_his_instance_12, old_other_instance_12
                    ]
                    ...
                    energy = (
                        self.interaction_energies[
                            old_his_instance_1, old_other_instance_2
                        ]
                        - self.interaction_energies[
                            old_his_instance_1, old_other_instance_12
                        ]
                        - self.interaction_energies[
                            old_his_instance_12, old_other_instance_2
                        ]
                    )
                    ...
                    energy = (
                        self.interaction_energies[
                            old_his_instance_2, old_other_instance_12
                        ]
                        - self.interaction_energies[
                            old_his_instance_12, old_other_instance_12
                        ]
                    ...
                    energy = self.interaction_energies[
                        old_his_instance_2, old_other_instance_2
                    ]
                    self.add_interaction_energy_pair(his_deprot, other_deprot, energy)

                combinations = self.get_interaction_combinations(
                    (old_his_instance_1, old_his_instance_2, old_his_instance_12),
            ...
            # Handle create interaction with HIe
                ...
                if his_name == "HIe":
                    ...
                    energy = (
                        self.interaction_energies[
                            old_his_instance_12, old_other_instance_1
                        ]
                        - self.interaction_energies[
                            old_his_instance_12, old_other_instance_12
                        ]
                    ...
                    energy = self.interaction_energies[
                        old_his_instance_1, old_other_instance_1
                    ]
                    self.add_interaction_energy_pair(his_deprot, other_deprot, energy)
                    ...
                    energy = (
                        self.interaction_energies[
                            old_his_instance_2, old_other_instance_1
                        ]
                        + self.interaction_energies[
                            old_his_instance_12, old_other_instance_12
                        ]
            ...
            # Handle create interaction with non-HIS
                ...
                if his_name == "HIe":
                    ...
                    energy = (
                        self.interaction_energies[old_his_instance_1, other_deprot]
    ...
    def evaluate_energy(self, labeling, normal_form=True, pH=0.0):
        ...
        if not normal_form:
            ...
            for v, w in combinations(iter(self.residue_variables.items()), 2):
                ...
                if is_same_his:
                    if (
                        labeling[v_residue].protonated
                        and labeling[w_residue].protonated
                    ):
                        ph_multiplier -= 2.0
    ...
    def evaluate_energy_diff(self, residue, labeling, normal_form=False):
        ...
        deprot_energy += (
            deprot_instance.energyNF if normal_form else deprot_instance.energy
        )

        return prot_energy - deprot_energy

    def evaluate_energy_diff_his(
        self, hie_residue, hid_residue, labeling, normal_form=False
    ):
    ...
    def normalize(self, pH):
        ...
        for residue in self.residue_variables.values():
            ...
            residue.instances["PROTONATED"].energyNF = (
                residue.instances["PROTONATED"].energy + math.log(10) * pH
            )
        ...
        for v_key, w_key in combinations(keys, 2):
            ...
            if is_same_his:
                ...
                self.normalized_interaction_energies[v_prot, w_prot] -= (
                    2.0 * math.log(10) * pH
                )
                self.normalized_interaction_energies[w_prot, v_prot] -= (
                    2.0 * math.log(10) * pH
                )
        ...
        for v_key, w_key in permutations(keys, 2):
            ...
            for v_instance in v_residue.instances.values():
                ...
                min_energy = min(
                    self.normalized_interaction_energies[v_instance, w_instance]
                ...
                if min_energy != sys.float_info.max:
                    for w_instance in w_instances:
                        self.normalized_interaction_energies[
                            v_instance, w_instance
                        ] -= min_energy
                        self.normalized_interaction_energies[
                            w_instance, v_instance
                        ] -= min_energy

        # Normalize the instance energies.
        for residue in self.residue_variables.values():
            min_energy = min(
                instance.energyNF for instance in residue.instances.values()
            )
    ...
    def energy_at_pH(self, pH):
        ...
        for residue in self.residue_variables.values():
            residue.instances["DEPROTONATED"].energy_with_ph = residue.instances[
                "DEPROTONATED"
            ].energy
            residue.instances["PROTONATED"].energy_with_ph = (
                residue.instances["PROTONATED"].energy + math.log(10) * pH
            )
        ...
        for v, w in combinations(iter(self.residue_variables.items()), 2):
            ...
            if is_same_his:
                v_prot = v_residue.instances["PROTONATED"]
                w_prot = w_residue.instances["PROTONATED"]
                self.interaction_energies_for_ph[v_prot, w_prot] -= (
                    2.0 * math.log(10) * pH
                )
                self.interaction_energies_for_ph[w_prot, v_prot] -= (
                    2.0 * math.log(10) * pH
                )
