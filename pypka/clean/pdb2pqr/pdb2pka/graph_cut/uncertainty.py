...
def resolve_uncertainty(
    protein_complex, labeling, uncertain, brute_force_limit=20, verbose=False
):
...
def brute_force(pc, labeling, uncertain):
    ...
    state_pairs = (
        (x.instances["PROTONATED"], x.instances["DEPROTONATED"]) for x in uncertain
    )
...
def monte_carlo(pc, labeling, uncertain):
    ...
    for _ in range(iterations):
        ...
        for _ in range(sub_iterations):
            ...
            diff = pc.evaluate_energy_diff(
                random_residue, test_labeling, normal_form=True
            )
            if diff < 0.0:
                ...
                test_labeling[random_residue] = random.choice(
                    list(random_residue.instances.values())
                )
