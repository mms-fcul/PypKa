...
def print_pc_state(pc, normal_form, out_file):
    ...
    ie = (
        pc.normalized_interaction_energies
        if normal_form
        else pc.interaction_energies_for_ph
    )
        ...
        for v_instance in v_residue.instances.values():
            for w_residue in rv.values():
                ...
                for w_instance in w_residue.instances.values():
                    out_file.write(
                        str((v_instance, w_instance))
                        + " "
                        + str(round(ie[v_instance, w_instance], 4))
                        + "\n"
                    )
    ...
    for key in keys:
        ...
        for instance in list(residue.instances.values()):
            if normal_form:
                out_file.write(
                    str(instance) + " " + str(round(instance.energyNF, 4)) + "\n"
                )
            else:
                out_file.write(
                    str(instance) + " " + str(round(instance.energy_with_ph, 4)) + "\n"
                )
    if normal_form:
        out_file.write(
            "Normalized constant energy: "
            + str(round(pc.normalized_constant_energy, 4))
            + "\n"
        )
...
def get_titration_curves(protein_complex, state_file=None):
    ...
    for step in range(steps):
        ...
        new_labeling = resolve_uncertainty(
            protein_complex, labeling, uncertain, verbose=True
        )
...
def get_curve_values(protein_complex, labeling, pH):
    ...
    for key, residue in protein_complex.residue_variables.items():
        ...
        if name in ("HId", "HIe"):
            ...
            energies.dGe, energies.dGd = protein_complex.evaluate_energy_diff_his(
                hie_residue, hid_residue, labeling, normal_form=True
            )
            ...
            energies.ddG = (energies.dGe + energies.dGeref) - (
                energies.dGd + energies.dGdref
            )
            ...
            energy_diff = protein_complex.evaluate_energy_diff(
                residue, labeling, normal_form=True
            )
