...
class pKaIO:
    ...
    def readpka(self, filename=None):
        ...
        while not done:
            ...
            linenumber = linenumber + 1
            if string.strip(string.lower(lines[linenumber])) == string.lower(
                "End of file"
            ):
                done = 1
        self.pka = pKa
    ...
    def write_pka(self, filename, data=None, format="WHAT IF"):
        ...
        fd.write("Format 1.0\n")
        fd.write(
            "      Group            pKa value  Model pK    dpK     dDesolv    dBack   dElec\n"
        )
        groups = data.keys()
        ...
        for group in groups:
            if data.has_key(group):
                ...
                fd.write(
                    "%15s      %7.4f  %7.4f  %7.4f  %7.4f  %7.4f  %7.4f  \n"
                    % (
    ...
    def readtitcurv(self, filename=None):
        ...
        # Parse
        #
        if string.lower(string.strip(lines[0])) == string.lower(
            "WHAT IF Titration Curve File"
        ):
            format = "WHAT IF"
        elif string.lower(string.strip(lines[0])) == string.lower(
            "pdb2pka Titration Curve File"
        ):
            format = "pdb2pka"
        else:
        ...
        while not done:
            ...
            charge = {"pKa": pKa}
            for pH in range(
                int(100 * phstart), int(100 * phend + 100 * phstep), int(100 * phstep)
            ):
                rpH = float(pH) / 100.0
    ...
    def read_matrix(self, filename=None):
        ...
        #
        if string.lower(string.strip(lines[0])) == string.lower(
            "WHAT IF Interaction Matrix File"
        ):
            format = "WHAT IF"
        elif string.lower(string.strip(lines[0])) == string.lower(
            "pdb2pka Interaction Matrix File"
        ):
            format = "WHAT IF"
        else:
            raise "Unknown format"
        ...
        while not done:
            ...
            if format == "WHAT IF":
                ...
                for count in range(partners):
                    ...
                    if len(list[3]) == 1:
                        partid = (
                            string.zfill(list[2], 4) + string.upper(list[1]) + list[3]
                        )
                    else:
    ...
    def write_matrix(self, filename, format="WHAT IF"):
        ...
        for group in newgroups:
            if self.matrix.has_key(group):
                ...
                if self.matrix.has_key("T" + group) and not written.has_key(
                    "T" + group
                ):
                    fd.write(
                        "%s      %7.4f\n"
                    ...
                    for group2 in sub_m.keys():
                        ...
                        for tit2 in sub_m[group2].keys():
                            for state2 in sub_m[group2][tit2].keys():
                                self.matrix[group1.uniqueid][group2.uniqueid].append(
                                    sub_m[group2][tit2][state2]
                                )
        ...
        for group2 in groups2:
            if self.matrix[group].has_key(group2):
                ...
                fd.write(
                    "%7.4f\n%7.4f\n%7.4f\n"
                    % (
                        self.matrix[group][group2][1],
                        self.matrix[group][group2][2],
                        self.matrix[group][group2][3],
                    )
                )
                written[group2] = 1
                #
                ...
                if self.matrix[group].has_key("T" + group2):
                    fd.write(
                        "%s      %7.4f\n"
                        % (
                            self.WI_res_text("T" + group2, format),
                            self.matrix[group]["T" + group2][0],
                        )
                    )
                    fd.write(
                        "%7.4f\n%7.4f\n%7.4f\n"
                        % (
                            self.matrix[group]["T" + group2][1],
                            self.matrix[group]["T" + group2][2],
                            self.matrix[group]["T" + group2][3],
                        )
                    )
                    written["T" + group2] = 1
            else:
                if self.matrix[group].has_key("T" + group2) and not written.has_key(
                    "T" + group2
                ):
                    fd.write(
                        "%s      %7.4f\n"
    ...
    def read_desolv(self, filename=None):
        ...
        #
        if (
            string.strip(lines[0]) == "WHAT IF Desolvation Energy File"
            and string.strip(lines[1]) == "Format 1.0"
        ):
            format = "WHAT IF"
