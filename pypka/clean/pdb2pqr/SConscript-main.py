...
vars.Add(
    PathVariable("PREFIX", "Install directory", defaultPrefix, PathVariable.PathAccept)
)
...
vars.Add(
    PathVariable(
        "APBS", "Location of the APBS binary if installed", "", PathVariable.PathAccept
    )
)
...
vars.Add(
    BoolVariable(
        "DEBUG", "Set to True to compiled components with debug headers.", False
    )
)
...
if env["BUILD_PDB2PKA"]:
    ...
    if not numpy_error:
        ...
        algorithms_pyc = env.LoadableModule(
            "pdb2pka/substruct/Algorithms", ["pdb2pka/substruct/Algorithms.cpp"]
        )
        ...
        if os.name == "nt":
            alg_msvs_env = env.Clone(
                MSVSBUILDCOM="cd .. && "
                + pythonBin
                + " scons/scons.py algorithms DEBUG=True",
                MSVSCLEANCOM="cd .. && " + pythonBin + " scons/scons.py -c algorithms",
                MSVSREBUILDCOM="cd .. && "
                + pythonBin
                + " scons/scons.py -c algorithms && "
                + pythonBin
                + " scons/scons.py algorithms DEBUG=True",
            )

            algorithms_project = alg_msvs_env.MSVSProject(
                target="msvs/Algorithms" + env["MSVSPROJECTSUFFIX"],
        ...
        if env["REBUILD_SWIG"]:
            pmc_pyc = env.LoadableModule(
                "pdb2pka/_pMC_mult", ["pdb2pka/pMC_mult.cpp", "pdb2pka/pMC_mult.i"]
            )
            build_swig = " REBUILD_SWIG=True"
        else:
            pmc_pyc = env.LoadableModule(
                "pdb2pka/_pMC_mult",
                ["pdb2pka/pMC_mult.cpp", "pdb2pka/pMC_mult_wrap.cpp"],
            )
            build_swig = " "
        ...
        if os.name == "nt":
            pmc_msvs_env = env.Clone(
                MSVSBUILDCOM="cd .. && "
                + pythonBin
                + " scons/scons.py pmc_mult DEBUG=True"
                + build_swig,
                MSVSCLEANCOM="cd .. && " + pythonBin + " scons/scons.py -c pmc_mult",
                MSVSREBUILDCOM="cd .. && "
                + pythonBin
                + " scons/scons.py -c pmc_mult && "
                + pythonBin
                + " scons/scons.py pmc_mult DEBUG=True"
                + build_swig,
            )

            pmc_mult_project = pmc_msvs_env.MSVSProject(
                target="msvs/pMC_mult" + env["MSVSPROJECTSUFFIX"],
                auto_build_solution=0,
            ...
            pdb2pqr_sln = env.MSVSSolution(
                target="msvs/pdb2pqr" + env["MSVSSOLUTIONSUFFIX"],
...
def print_default_message(target_list):
    ...
    if not GetOption("help"):
        ...
        if env["OPAL"] == "":
            ...
            print "PDB2PQR jobs run via the web interface will be run via opal at", env[
                "OPAL"
            ]
    else:
        print
    ...
    print
    print "To setup a web service create a symbolic link to", env[
        "PREFIX"
    ], "that enables you to view", env["URL"], 'after running "scons/scons.py install"'
    print
