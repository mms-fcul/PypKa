...
def installFile(file_name, build_target="install"):
    ...
    if file_name.endswith(".py"):
        ...
        result = env.Command(
            compiled_name, file_name, CompilePython("$TARGET", "$SOURCE")
        )
        if GetOption("clean"):
            env.Default(result)
...
# Main Program
for dir_name in ("src/", "extensions/"):
...
Alias(
    "install",
    env.Command(
        env["PREFIX"] + "tmp", None, [Mkdir("$TARGET"), Chmod("$TARGET", 0777)]
    ),
)

# Testing stuff to test installed pdb2pqr
for dir_name in ("tests/", "scons/", "site_scons/"):
    for file_name in getAllFiles(dir_name):
...
Alias(
    "install-tests",
    env.Install(env["PREFIX"] + "site_scons/", "site_scons/site_init.py"),
)
